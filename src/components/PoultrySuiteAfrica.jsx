...
// ── Shared Sales & Procurement logic (used by all three module reducers) ──
// Handles customers, orders, order payments, suppliers, purchase orders and
// vendor payments. The professional rule: Financials reflects ACTUAL CASH.
// So every payment (customer payment or vendor payment) posts a financialLog
// (Revenue for sales, Cost for procurement), tagged with sourceType/sourceId
// so it can be removed if the payment is deleted — never double-counted.
// Returns the new state (without audit; the calling reducer adds its own audit).
function applySalesProcurement(state, action, uidFn) {
  const u = uidFn || ((p) => `${p}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,5)}`);
  const fin = state.financialLogs || [];
  const without = (sourceType, sourceId) => fin.filter(f => !(f.sourceType === sourceType && f.sourceId === sourceId));
  switch (action.type) {
    // Customers
    case 'SP_ADD_CUSTOMER': return { ...state, customers: [action.p, ...(state.customers || [])] };
    case 'SP_UPDATE_CUSTOMER': return { ...state, customers: (state.customers || []).map(c => c.id === action.p.id ? action.p : c) };
    case 'SP_DELETE_CUSTOMER': return { ...state, customers: (state.customers || []).filter(c => c.id !== action.p) };
    // Orders (sales)
    case 'SP_ADD_ORDER': return { ...state, orders: [action.p, ...(state.orders || [])] };
    case 'SP_UPDATE_ORDER': return { ...state, orders: (state.orders || []).map(o => o.id === action.p.id ? action.p : o) };
    case 'SP_DELETE_ORDER': {
      // Remove the order and any revenue logs from its payments.
      const newFin = fin.filter(f => !(f.sourceType === 'order_payment' && f.orderId === action.p));
      return { ...state, orders: (state.orders || []).filter(o => o.id !== action.p), financialLogs: newFin };
    }
    // Customer payment against an order -> Revenue financialLog (actual cash in)
    case 'SP_ADD_ORDER_PAYMENT': {
      const { order, payment } = action.p; // payment: {id,date,amount,method,notes}
      const orders = (state.orders || []).map(o => o.id === order.id ? { ...o, payments: [...(o.payments || []), payment] } : o);
      const log = { id: u('F'), batchId: order.batchId || null, date: payment.date, type: 'Revenue', category: 'Sales', amount: Number(payment.amount) || 0, qty: order.qty || 0, notes: `Payment for order ${order.id} (${order.product})`, sourceType: 'order_payment', sourceId: payment.id, orderId: order.id };
      return { ...state, orders, financialLogs: [log, ...fin] };
    }
    case 'SP_DELETE_ORDER_PAYMENT': {
      const { orderId, paymentId } = action.p;
      const orders = (state.orders || []).map(o => o.id === orderId ? { ...o, payments: (o.payments || []).filter(p => p.id !== paymentId) } : o);
      return { ...state, orders, financialLogs: without('order_payment', paymentId) };
    }
    // Suppliers
    case 'SP_ADD_SUPPLIER': return { ...state, suppliers: [action.p, ...(state.suppliers || [])] };
    case 'SP_UPDATE_SUPPLIER': return { ...state, suppliers: (state.suppliers || []).map(s => s.id === action.p.id ? action.p : s) };
    case 'SP_DELETE_SUPPLIER': return { ...state, suppliers: (state.suppliers || []).filter(s => s.id !== action.p) };
    // Purchase orders (procurement)
    case 'SP_ADD_PO': return { ...state, purchaseOrders: [action.p, ...(state.purchaseOrders || [])] };
    case 'SP_UPDATE_PO': return { ...state, purchaseOrders: (state.purchaseOrders || []).map(p => p.id === action.p.id ? action.p : p) };
    case 'SP_DELETE_PO': {
      const newFin = fin.filter(f => !(f.sourceType === 'po_payment' && f.poId === action.p));
      return { ...state, purchaseOrders: (state.purchaseOrders || []).filter(p => p.id !== action.p), financialLogs: newFin };
    }
    // Vendor payment against a PO -> Cost financialLog (actual cash out)
    case 'SP_ADD_PO_PAYMENT': {
      const { po, payment } = action.p;
      const purchaseOrders = (state.purchaseOrders || []).map(p => p.id === po.id ? { ...p, payments: [...(p.payments || []), payment] } : p);
      const log = { id: u('F'), batchId: null, date: payment.date, type: 'Cost', category: 'Procurement', amount: Number(payment.amount) || 0, notes: `Payment to ${po.supplierName || 'supplier'} for PO ${po.id} (${po.item})`, sourceType: 'po_payment', sourceId: payment.id, poId: po.id };
      return { ...state, purchaseOrders, financialLogs: [log, ...fin] };
    }
    case 'SP_DELETE_PO_PAYMENT': {
      const { poId, paymentId } = action.p;
      const purchaseOrders = (state.purchaseOrders || []).map(p => p.id === poId ? { ...p, payments: (p.payments || []).filter(x => x.id !== paymentId) } : p);
      return { ...state, purchaseOrders, financialLogs: without('po_payment', paymentId) };
    }
...
function fmReducer(state,action){
  ...
  switch(action.type){
    ...
    case 'FM_ADD_CUSTOMER':{const s={...state,customers:[action.p,...(state.customers||[])]};return audit(s,`Customer added: ${action.p.name}`,'Customer',null,action.p);}
    case 'FM_UPDATE_CUSTOMER':{const s={...state,customers:(state.customers||[]).map(c=>c.id===action.p.id?action.p:c)};return audit(s,`Customer updated: ${action.p.name}`,'Customer',null,action.p);}
    case 'FM_DELETE_CUSTOMER':{const s={...state,customers:(state.customers||[]).filter(c=>c.id!==action.p)};return audit(s,'Customer deleted','Customer',null,null);}
    case 'FM_ADD_ORDER':{const s={...state,orders:[action.p,...(state.orders||[])]};return audit(s,`Order: ${action.p.qty}kg ${action.p.product}`,'Order',null,action.p);}
    case 'FM_UPDATE_ORDER':{const s={...state,orders:(state.orders||[]).map(o=>o.id===action.p.id?action.p:o)};return audit(s,`Order ${action.p.id}: ${action.p.status}`,'Order',null,action.p);}
    case 'FM_DELETE_ORDER':{const s={...state,orders:(state.orders||[]).filter(o=>o.id!==action.p)};return audit(s,'Order deleted','Order',null,null);}
    ...
    default:
      if(action.type&&action.type.indexOf('SP_')===0){
        const ns=applySalesProcurement(state,action,fmuid);
        if(ns!==state)return audit(ns,`Sales/Procurement: ${action.type.replace('SP_','').toLowerCase().replace(/_/g,' ')}`,'SalesProcurement',null,null);
      }
      return state;
  }
}
...
function hsReducer(state,action){
  ...
  switch(action.type){
    ...
    case 'HS_ADD_CUSTOMER':{const s={...state,customers:[action.p,...(state.customers||[])]};return audit(s,`Customer added: ${action.p.name}`,'Customer',null,action.p);}
    case 'HS_UPDATE_CUSTOMER':{const s={...state,customers:(state.customers||[]).map(c=>c.id===action.p.id?action.p:c)};return audit(s,`Customer updated: ${action.p.name}`,'Customer',null,action.p);}
    case 'HS_DELETE_CUSTOMER':{const s={...state,customers:(state.customers||[]).filter(c=>c.id!==action.p)};return audit(s,'Customer deleted','Customer',null,null);}
    ...
    default:
      if(action.type&&action.type.indexOf('SP_')===0){
        const ns=applySalesProcurement(state,action,huid);
        if(ns!==state)return audit(ns,`Sales/Procurement: ${action.type.replace('SP_','').toLowerCase().replace(/_/g,' ')}`,'SalesProcurement',null,null);
      }
      return state;
  }
...
function psReducer(state,action){
  ...
  switch(action.type){
    ...
    case '__SYNC_SET_BATCHES':{return {...state,batches:action.p};}
    case '__SYNC_SET_COLLECTION':{return {...state,[action.collection]:action.p};}
    ...
    // ── Quarantine & Biosecurity engine ──
    case 'BIO_ADD_CASE':{const s={...state,bioCases:[action.p,...(state.bioCases||[])],batches:action.p.batchId?state.batches.map(b=>b.id===action.p.batchId?{...b,status:'Quarantined'}:b):state.batches};return audit(s,`Quarantine case opened: ${action.p.title}`,'Biosecurity',action.p.id,null,action.p);}
    case 'BIO_UPDATE_CASE':{const s={...state,bioCases:(state.bioCases||[]).map(c=>c.id===action.p.id?action.p:c)};return audit(s,`Case updated: ${action.p.title}`,'Biosecurity',action.p.id,null,action.p);}
    case 'BIO_DELETE_CASE':{const s={...state,bioCases:(state.bioCases||[]).filter(c=>c.id!==action.p)};return audit(s,'Quarantine case deleted','Biosecurity',action.p,null,null);}
    case 'BIO_RESOLVE_CASE':{
      // action.p = {caseId, resolution:{dead,returned,culled,stillQ}, rootCause, recommendation, date}
      const r=action.p; const c=(state.bioCases||[]).find(x=>x.id===r.caseId); if(!c)return state;
      let batches=state.batches; let mortalityLogs=state.mortalityLogs; let inventory=state.inventory; let financialLogs=state.financialLogs;
      const dead=Number(r.resolution.dead)||0, returned=Number(r.resolution.returned)||0, culled=Number(r.resolution.culled)||0, stillQ=Number(r.resolution.stillQ)||0;
      // Dead birds -> Mortality engine (and reduce batch count)
      if(dead>0 && c.batchId){
        mortalityLogs=[{id:uid('M'),batchId:c.batchId,date:r.date,count:dead,cause:'Disease',notes:`From quarantine case ${c.id}`,photos:[]},...mortalityLogs];
        batches=batches.map(b=>b.id===c.batchId?{...b,currentCount:Math.max(0,(b.currentCount||0)-dead)}:b);
      }
      // Culled birds -> reduce batch count + financial loss
      if(culled>0 && c.batchId){
        batches=batches.map(b=>b.id===c.batchId?{...b,currentCount:Math.max(0,(b.currentCount||0)-culled)}:b);
        const lossPer=Number(r.cullValuePer)||0;
        if(lossPer>0)financialLogs=[{id:uid('F'),batchId:c.batchId,date:r.date,type:'Cost',category:'Quarantine Loss',amount:culled*lossPer,notes:`Culled ${culled} birds (case ${c.id})`},...financialLogs];
      }
      // Returned birds rejoin batch (no count change — they were never removed from population, just isolated)
      // If the whole case is resolved (nothing still in Q) and batch was quarantined, restore batch status
      if(stillQ===0 && c.batchId){
        batches=batches.map(b=>b.id===c.batchId&&b.status==='Quarantined'?{...b,status:'Active'}:b);
      }
      const resolved={...c,status:stillQ>0?'Active':'Resolved',resolution:r.resolution,rootCause:r.rootCause,recommendation:r.recommendation,resolvedDate:stillQ>0?null:r.date};
      const s={...state,bioCases:(state.bioCases||[]).map(x=>x.id===c.id?resolved:x),batches,mortalityLogs,inventory,financialLogs};
      return audit(s,`Case resolved: ${c.title} (dead ${dead}, culled ${culled}, returned ${returned})`,'Biosecurity',c.id,null,resolved);
    }
    case 'BIO_ADD_ZONE':{const s={...state,bioZones:[action.p,...(state.bioZones||[])]};return audit(s,`Isolation zone: ${action.p.name}`,'Biosecurity',action.p.id,null,action.p);}
    case 'BIO_UPDATE_ZONE':{const s={...state,bioZones:(state.bioZones||[]).map(z=>z.id===action.p.id?action.p:z)};return audit(s,`Zone updated: ${action.p.name}`,'Biosecurity',action.p.id,null,action.p);}
    case 'BIO_DELETE_ZONE':{const s={...state,bioZones:(state.bioZones||[]).filter(z=>z.id!==action.p)};return audit(s,'Zone deleted','Biosecurity',action.p,null,null);}
    case 'BIO_ADD_SANITATION':{const s={...state,bioSanitation:[action.p,...(state.bioSanitation||[])]};return audit(s,`Sanitation: ${action.p.area}`,'Biosecurity',action.p.id,null,action.p);}
    case 'BIO_DELETE_SANITATION':{const s={...state,bioSanitation:(state.bioSanitation||[]).filter(x=>x.id!==action.p)};return audit(s,'Sanitation log deleted','Biosecurity',action.p,null,null);}
    case 'BIO_ADD_ACCESS':{const s={...state,bioAccess:[action.p,...(state.bioAccess||[])]};return audit(s,`Access log: ${action.p.name} (${action.p.type})`,'Biosecurity',action.p.id,null,action.p);}
    case 'BIO_DELETE_ACCESS':{const s={...state,bioAccess:(state.bioAccess||[]).filter(x=>x.id!==action.p)};return audit(s,'Access log deleted','Biosecurity',action.p,null,null);}
    case 'BIO_SET_LOCKDOWN':{const s={...state,bioLockdown:action.p};return audit(s,`Farm lockdown ${action.p?'ENABLED':'lifted'}`,'Biosecurity',null,null,action.p);}
    case 'BIO_SET_COMPLIANCE':{const s={...state,bioCompliance:action.p};return audit(s,'Biosecurity checklist updated','Biosecurity',null,null,null);}
    case 'PS_ADD_CUSTOMER':{const s={...state,customers:[action.p,...(state.customers||[])]};return audit(s,`Customer added: ${action.p.name}`,'Customer',null,null,action.p);}
    case 'PS_UPDATE_CUSTOMER':{const s={...state,customers:(state.customers||[]).map(c=>c.id===action.p.id?action.p:c)};return audit(s,`Customer updated: ${action.p.name}`,'Customer',null,null,action.p);}
    case 'PS_DELETE_CUSTOMER':{const s={...state,customers:(state.customers||[]).filter(c=>c.id!==action.p)};return audit(s,'Customer deleted','Customer',null,null,null);}
    case 'PS_ADD_ORDER':{const s={...state,orders:[action.p,...(state.orders||[])]};return audit(s,`Order: ${action.p.qty} ${action.p.product}`,'Order',null,null,action.p);}
    case 'PS_UPDATE_ORDER':{const s={...state,orders:(state.orders||[]).map(o=>o.id===action.p.id?action.p:o)};return audit(s,`Order ${action.p.id}: ${action.p.status}`,'Order',null,null,action.p);}
    case 'PS_DELETE_ORDER':{const s={...state,orders:(state.orders||[]).filter(o=>o.id!==action.p)};return audit(s,'Order deleted','Order',null,null,null);}
    // ── Veterinary & Medical engine ──
    case 'VET_ADD_TREATMENT':{const s={...state,vetTreatments:[action.p,...(state.vetTreatments||[])]};return audit(s,`Treatment: ${action.p.condition}`,'Veterinary',action.p.id,null,action.p);}
    case 'VET_UPDATE_TREATMENT':{const s={...state,vetTreatments:(state.vetTreatments||[]).map(t=>t.id===action.p.id?action.p:t)};return audit(s,`Treatment updated: ${action.p.condition}`,'Veterinary',action.p.id,null,action.p);}
    case 'VET_DELETE_TREATMENT':{const s={...state,vetTreatments:(state.vetTreatments||[]).filter(t=>t.id!==action.p)};return audit(s,'Treatment deleted','Veterinary',action.p,null,null);}
    case 'VET_ADD_DRUG':{const s={...state,vetDrugs:[action.p,...(state.vetDrugs||[])]};return audit(s,`Drug added: ${action.p.name}`,'Veterinary',action.p.id,null,action.p);}
    case 'VET_UPDATE_DRUG':{const s={...state,vetDrugs:(state.vetDrugs||[]).map(d=>d.id===action.p.id?action.p:d)};return audit(s,`Drug updated: ${action.p.name}`,'Veterinary',action.p.id,null,action.p);}
    case 'VET_DELETE_DRUG':{const s={...state,vetDrugs:(state.vetDrugs||[]).filter(d=>d.id!==action.p)};return audit(s,'Drug deleted','Veterinary',action.p,null,null);}
    case 'VET_ADD_PRESCRIPTION':{const s={...state,vetPrescriptions:[action.p,...(state.vetPrescriptions||[])]};return audit(s,`Prescription: ${action.p.drug}`,'Veterinary',action.p.id,null,action.p);}
    case 'VET_UPDATE_PRESCRIPTION':{const s={...state,vetPrescriptions:(state.vetPrescriptions||[]).map(p=>p.id===action.p.id?action.p:p)};return audit(s,`Prescription updated`,'Veterinary',action.p.id,null,action.p);}
    case 'VET_DELETE_PRESCRIPTION':{const s={...state,vetPrescriptions:(state.vetPrescriptions||[]).filter(p=>p.id!==action.p)};return audit(s,'Prescription deleted','Veterinary',action.p,null,null);}
    case 'VET_ADD_CONSULT':{const s={...state,vetConsults:[action.p,...(state.vetConsults||[])]};return audit(s,`Vet consult: ${action.p.vetName}`,'Veterinary',action.p.id,null,action.p);}
    case 'VET_UPDATE_CONSULT':{const s={...state,vetConsults:(state.vetConsults||[]).map(c=>c.id===action.p.id?action.p:c)};return audit(s,'Vet consult updated','Veterinary',action.p.id,null,action.p);}
    case 'VET_DELETE_CONSULT':{const s={...state,vetConsults:(state.vetConsults||[]).filter(c=>c.id!==action.p)};return audit(s,'Vet consult deleted','Veterinary',action.p,null,null);}
    ...
    default:
      if(action.type&&action.type.indexOf('SP_')===0){
        const ns=applySalesProcurement(state,action,uid);
        if(ns!==state)return audit(ns,`Sales/Procurement: ${action.type.replace('SP_','').toLowerCase().replace(/_/g,' ')}`,'SalesProcurement',null,null,null);
      }
      return state;
  }
}
...
const T = {
  ...
  chartBlue:"#6B9BD1", chartBlueSoft:"#A9C8E8",
...
.mono{font-family:inherit;letter-spacing:-0.01em;}
      ...
      <div style={{width:Math.min(420,window?.innerWidth||420),height:'100vh',background:T.bg0,display:'flex',flexDirection:'column',boxShadow:'-8px 0 32px rgba(0,0,0,0.15)'}}>
        ...
        <div style={{flex:1,overflowY:'auto'}}>
          {filtered.length===0?(
            ...
            const canNav = n.actionEngine && onNavigate;
            const go = () => { if(canNav){ onNavigate(n.mod,n.actionEngine,n.actionType,n.actionContext); onClose(); } };
            return(
            <div key={i} onClick={canNav?go:undefined}
              style={{padding:'14px 20px',borderBottom:`1px solid ${T.line}`,background:priBg[n.pri]||'#fff',display:'flex',gap:12,alignItems:'flex-start',cursor:canNav?'pointer':'default',transition:'background .12s'}}
              onMouseEnter={canNav?(e=>{e.currentTarget.style.filter='brightness(0.97)';}):undefined}
              onMouseLeave={canNav?(e=>{e.currentTarget.style.filter='none';}):undefined}
              role={canNav?'button':undefined} tabIndex={canNav?0:undefined}
              onKeyDown={canNav?(e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();go();}}):undefined}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:13,color:T.ink,lineHeight:1.45,marginBottom:canNav?8:0}}>{n.msg}</div>
                {canNav&&(
                  <div style={{display:'flex',alignItems:'center',gap:6}}>
                    <span style={{fontSize:12,fontWeight:600,color:priColor[n.pri]||T.ink}}>{n.actionLabel||'Open'}</span>
                    <svg width={13} height={13} viewBox="0 0 24 24" fill="none" stroke={priColor[n.pri]||T.ink} strokeWidth="2" strokeLinecap="square" strokeLinejoin="miter"><path d="M9 18l6-6-6-6"/></svg>
                  </div>
                )}
              </div>
              {canNav&&(
                <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke={T.ink4} strokeWidth="1.75" strokeLinecap="square" strokeLinejoin="miter" style={{flexShrink:0,alignSelf:'center'}}><path d="M9 18l6-6-6-6"/></svg>
              )}
            </div>
          );})}
        </div>
...
function EngineIcon({id,size=22,color="currentColor"}){
  ...
  switch(id){
    ...
    case "vet":return(<svg width={size} height={size} viewBox={v} {...p}>
      <rect x="3" y="8" width="18" height="11" rx="1"/>
      <path d="M8 8V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v3"/>
      <path d="M12 11v5M9.5 13.5h5"/>
    </svg>);
    case "bio":return(<svg width={size} height={size} viewBox={v} {...p}>
      <path d="M12 2L4 5v6c0 5 3.5 8 8 11 4.5-3 8-6 8-11V5z"/>
...
function HelpSectionIcon({id, size=18, color}){
  ...
  switch(id){
    ...
    case "devices":return(<svg width={size} height={size} viewBox={v} {...p}>
      <rect x="2" y="4" width="13" height="10"/>
      <path d="M2 18H10"/>
      <rect x="16" y="9" width="6" height="11"/>
      <path d="M18.5 17H19.5"/>
    </svg>);
    case "cloud":return(<svg width={size} height={size} viewBox={v} {...p}>
...
const addDays=(d,n)=>{const dt=new Date(d);if(isNaN(dt))return '';dt.setDate(dt.getDate()+Number(n||0));return dt.toISOString().split('T')[0];};
...
const POULTRY_ENGINE_RES={
  ...
  vet:'poultry.operations',
  bio:'poultry.operations',
  orders:'poultry.operations',
...
function UserManagementPanel({license,activeUser,onUpdateLicense}){
  ...
  return(<div style={{display:'flex',flexDirection:'column',gap:14}}>
    <div style={{background:T.accentBg,border:`1px solid ${T.accentLine}`,padding:'12px 14px',fontSize:12,color:T.accentDark,lineHeight:1.5}}>
      Each user signs in with their own 6-digit PIN. Their role determines what they can see and edit across all modules. The Owner cannot be removed.
    </div>
    ...
    {editing!==null&&canWrite&&(<div style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px'}}>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:10}}>
        ...
        <Inp label="6-Digit PIN" value={form.pin} onChange={v=>setForm(f=>({...f,pin:v.replace(/\D/g,'').slice(0,6)}))} placeholder="000000" hint="Used to sign in to this device"/>
...
function KPICard({label,value,unit,sub,alert}){
  ...
  const fs=str.length>12?13:str.length>8?16:18;
  return(<div style={{background:T.bg0,border:`1px solid ${alert?T.errLine:T.line}`,padding:"13px 15px",borderLeft:`3px solid ${alert?T.err:T.accentLine}`}}>
    ...
    <div className="mono" style={{fontSize:fs,fontWeight:700,color:alert?T.err:T.ink,lineHeight:1,wordBreak:'break-word'}}>{value}{unit&&<span style={{fontSize:11,color:T.ink4,marginLeft:3,fontWeight:400}}>{unit}</span>}</div>
    {sub&&<div style={{fontSize:11,color:T.ink4,marginTop:4}}>{sub}</div>}
  </div>);
}

// ── Lightweight inline-SVG charts (no external dependency) ──
// data: [{label, value}]. color defaults to accent. Renders responsive bars.
function MiniBarChart({data=[],color,height=120,unit='',valueFmt}){
  const c=color||T.accent;
  if(!data.length)return<div style={{fontSize:12,color:T.ink4,padding:'24px 0',textAlign:'center'}}>No data yet</div>;
  const max=Math.max(1,...data.map(d=>d.value||0));
  const fmt=valueFmt||((v)=>v);
  const n=data.length;const gap=6;const bw=Math.max(6,(100-(n-1)*0)/n);
  return(
    <div>
      <div style={{display:'flex',alignItems:'flex-end',gap:gap,height,padding:'0 2px'}}>
        {data.map((d,i)=>{const h=Math.round(((d.value||0)/max)*100);return(
          <div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-end',height:'100%',minWidth:0}}>
            <div style={{fontSize:9,color:T.ink4,marginBottom:3,whiteSpace:'nowrap'}} className="mono">{(d.value||0)>0?fmt(d.value):''}</div>
            <div title={`${d.label}: ${fmt(d.value)}${unit}`} style={{width:'100%',maxWidth:38,height:`${h}%`,minHeight:(d.value||0)>0?2:0,background:c,transition:'height .3s',borderRadius:'2px 2px 0 0'}}/>
          </div>
        );})}
      </div>
      <div style={{display:'flex',gap:gap,marginTop:6,borderTop:`1px solid ${T.line}`,paddingTop:5}}>
        {data.map((d,i)=><div key={i} style={{flex:1,fontSize:9,color:T.ink4,textAlign:'center',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{d.label}</div>)}
      </div>
    </div>
  );
}

// data: [{label, value}]. Renders a smooth-ish line/area trend in SVG.
function MiniLineChart({data=[],color,height=120,unit='',valueFmt}){
  const c=color||T.accent;
  if(data.length<2)return<div style={{fontSize:12,color:T.ink4,padding:'24px 0',textAlign:'center'}}>Not enough data to chart yet</div>;
  const fmt=valueFmt||((v)=>v);
  const W=320,H=height,pad=6;
  const max=Math.max(1,...data.map(d=>d.value||0));
  const min=Math.min(0,...data.map(d=>d.value||0));
  const span=max-min||1;
  const stepX=(W-pad*2)/(data.length-1);
  const pts=data.map((d,i)=>{const x=pad+i*stepX;const y=pad+(1-((d.value||0)-min)/span)*(H-pad*2);return[x,y];});
  ...
  return(
    <div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',marginTop:4,borderTop:`1px solid ${T.line}`,paddingTop:5}}>
        <span style={{fontSize:9,color:T.ink4}}>{data[0].label}</span>
        <span style={{fontSize:9,color:T.ink4}}>peak {fmt(max)}{unit}</span>
        <span style={{fontSize:9,color:T.ink4}}>{data[data.length-1].label}</span>
      </div>
    </div>
  );
}
...
function ChartCard({title,subtitle,children}){
  return(<div style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px'}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline',marginBottom:12}}>
      <span style={{fontSize:12,fontWeight:700,color:T.ink4,textTransform:'uppercase',letterSpacing:0.7}}>{title}</span>
      {subtitle&&<span style={{fontSize:11,color:T.ink4}}>{subtitle}</span>}
    </div>
    {children}
  </div>);
}

// Bucket records into the last N days; sumFn extracts the numeric value.
function lastNDaysBuckets(records,dateKey,valueFn,n=7){
  const days=[];const today=new Date();
  for(let i=n-1;i>=0;i--){const d=new Date(today);d.setDate(d.getDate()-i);days.push(d.toISOString().split('T')[0]);}
  return days.map(day=>{
    const v=(records||[]).filter(r=>(r[dateKey]||'').slice(0,10)===day).reduce((s,r)=>s+(valueFn?valueFn(r):1),0);
    const dd=new Date(day);
    return {label:`${dd.getDate()}/${dd.getMonth()+1}`,value:v,day};
  });
}
      ...
      <div style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'22px 22px'}} className="au">
        ...
        {step===2&&(<>
          ...
          <div style={{marginTop:18,background:T.bg2,border:`1px solid ${T.line}`,padding:'18px 16px 22px',textAlign:'center'}}>
            <PinPad length={6} onComplete={handlePin} label={pinPhase==='set'?'Create 6-digit PIN':'Confirm your PIN'} hint={pinPhase==='set'?'Authenticates your role on this device':'Re-enter your PIN to confirm'}/>
          </div>
...
function PinLoginScreen({storedPin,license,onSuccess,onReset}){
    ...
    <div style={{minHeight:'100vh',background:T.bg1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:16}}>
      <div style={{width:'100%',maxWidth:340}} className="au">
        ...
        <div style={{background:T.bg0,border:`1px solid ${T.line}`,padding:22}}>
          ...
          {locked?<Notice type="error" message="Device locked. Please restart the application to reset."/>:<PinPad length={6} onComplete={handlePin} label="Enter PIN"/>}
          {error&&!locked&&<div style={{marginTop:13}}><Notice type="error" message={error}/></div>}
          {onReset&&<div style={{marginTop:18,paddingTop:16,borderTop:`1px solid ${T.line}`}}>
            {!confirmReset?(
              <button onClick={()=>setConfirmReset(true)} style={{width:'100%',background:'transparent',border:'none',color:T.ink4,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'inherit',padding:'4px'}}>
                Can't sign in? Reset device setup
              </button>
            ):(
              <div style={{textAlign:'center'}}>
                <div style={{fontSize:12,color:T.ink3,lineHeight:1.5,marginBottom:12}}>This re-runs onboarding so you can set a new PIN. Your farm data stays safe in your account — only this device's setup is cleared.</div>
                <div style={{display:'flex',gap:8}}>
                  <button onClick={()=>setConfirmReset(false)} style={{flex:1,background:T.bg0,border:`1px solid ${T.line}`,color:T.ink3,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'inherit',padding:'10px'}}>Cancel</button>
                  <button onClick={onReset} style={{flex:1,background:T.accent,border:'none',color:'#fff',fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit',padding:'10px'}}>Reset setup</button>
                </div>
              </div>
            )}
          </div>}
...
// ══════════════════════════════════════════════════════════
// First-run onboarding overlay for a brand-new farm. Shows once (per device),
// dismissible. Lets the owner set farm basics and guides them to the first
// house + batch. Stored dismissal in localStorage so it doesn't nag.
function OnboardingFlow({state,dispatch,onClose}){
  const [step,setStep]=useState(0);
  const [farm,setFarm]=useState({orgName:state.settings?.orgName||'',location:state.settings?.location||''});
  const [house,setHouse]=useState({name:'',capacity:''});
  const steps=[
    {title:'Welcome to PoultrySuite Africa',
     body:'Let\'s get your farm set up in a few quick steps. You can skip at any time and do this later from the relevant engines.'},
    {title:'Your Farm Details',
     body:'Tell us your farm name and location. This appears on your reports and helps personalise the app.'},
    {title:'Add Your First House',
     body:'A "house" is a physical building or pen. You\'ll place batches of birds inside houses. Add your first one now (optional).'},
    {title:'You\'re Ready',
     body:'That\'s the basics! From here: create a batch in Houses & Batches, then record mortality, feed, and vaccinations daily. The Help section has full guides whenever you need them.'},
  ];
  const cur=steps[step];
  const last=step===steps.length-1;

  const saveFarm=()=>{dispatch({type:'UPDATE_SETTINGS',p:{orgName:farm.orgName,location:farm.location}});setStep(2);};
  const saveHouse=()=>{
    if(house.name){dispatch({type:'ADD_HOUSE',p:{id:uid('H'),name:house.name,capacity:Number(house.capacity)||0,notes:''}});}
    setStep(3);
  };
  const finish=()=>{try{localStorage.setItem('psa::onboarded','1');}catch(_){}onClose();};

  return(
    <div style={{position:'fixed',inset:0,zIndex:9000,background:'rgba(17,24,39,0.55)',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
      <div style={{background:T.bg0,width:'100%',maxWidth:460,border:`1px solid ${T.line}`,boxShadow:'0 12px 40px rgba(0,0,0,0.22)',overflow:'hidden'}}>
        <div style={{background:`linear-gradient(135deg, ${T.accent} 0%, ${T.accent}D9 100%)`,padding:'22px 24px'}}>
          <div style={{fontSize:10,fontWeight:700,color:'rgba(255,255,255,0.85)',textTransform:'uppercase',letterSpacing:1.4,marginBottom:6}}>Step {step+1} of {steps.length}</div>
          <div style={{fontSize:19,fontWeight:800,color:'#fff',letterSpacing:-0.3,lineHeight:1.2}}>{cur.title}</div>
        </div>
        <div style={{display:'flex',gap:5,padding:'14px 24px 0'}}>
          {steps.map((_,i)=><div key={i} style={{flex:1,height:3,background:i<=step?T.accent:T.line}}/>)}
        </div>
        <div style={{padding:'18px 24px 22px'}}>
          <div style={{fontSize:13,color:T.ink3,lineHeight:1.65,marginBottom:18}}>{cur.body}</div>
          {step===1&&<div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:18}}>
            <Inp label="Farm Name" value={farm.orgName} onChange={v=>setFarm(f=>({...f,orgName:v}))} placeholder="e.g. Ipadeola Agoro Farms"/>
            <Inp label="Location" value={farm.location} onChange={v=>setFarm(f=>({...f,location:v}))} placeholder="e.g. Ibadan, Oyo State"/>
          </div>}
          {step===2&&<div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:18}}>
            <Inp label="House Name" value={house.name} onChange={v=>setHouse(h=>({...h,name:v}))} placeholder="e.g. House A"/>
            <Inp label="Capacity (birds)" type="number" value={house.capacity} onChange={v=>setHouse(h=>({...h,capacity:v}))} placeholder="e.g. 2000"/>
          </div>}
          <div style={{display:'flex',gap:10,alignItems:'center'}}>
            {step===0&&<Btn full onClick={()=>setStep(1)}>Get Started</Btn>}
            {step===1&&<Btn full onClick={saveFarm}>Continue</Btn>}
            {step===2&&<Btn full onClick={saveHouse}>{house.name?'Add House & Continue':'Skip This Step'}</Btn>}
            {last&&<Btn full onClick={finish}>Start Using PoultrySuite</Btn>}
          </div>
          {!last&&<button onClick={finish} style={{width:'100%',marginTop:10,background:'transparent',border:'none',color:T.ink4,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>Skip onboarding</button>}
        </div>
      </div>
    </div>
  );
...
// Shared "Accounts & Devices" section — identical across all modules, since
// accounts, sync, and device pairing are app-wide capabilities.
const HELP_ACCOUNTS_SECTION={
  id:'accounts',label:'Accounts & Devices',icon:'devices',
  topics:[
    {
      title:'Your Farm Account & Cloud Sync',
      preview:'How your account works and how data is shared across your devices.',
      body:'PoultrySuite Africa is built around your farm account. When you sign up, a farm is created for you automatically, and you become its owner.\n\nAll your operational data — flocks, mortality, feed, eggs, hatchery records, feed mill production, and financials — is stored securely in your farm\'s cloud account, not just on one device. This means:\n\nEvery device you register sees the same live data. A mortality record entered on a tablet in the layer house appears on the office tablet too.\n\nThe app works fully offline. If a device loses internet, you can keep recording — entries are saved on the device and sync automatically the moment the connection returns.\n\nYour data is private to your farm. Other farms using PoultrySuite cannot see it, and you cannot see theirs. This is enforced at the database level.',
      terms:[
        {t:'Farm Account',d:'The central account that owns all your data. Created automatically when you sign up.'},
        {t:'Owner',d:'The person who created the farm account. Owners can register and remove devices.'},
        {t:'Cloud Sync',d:'The automatic sharing of your data across all your registered devices via your farm account.'},
        {t:'Offline Mode',d:'The ability to keep working with no internet. Changes queue locally and sync on reconnect.'},
      ]
    },
    {
      title:'Registering Devices with Pairing Codes',
      preview:'How to add a new tablet to your farm using a pairing code.',
      body:'Your subscription plan includes a set number of devices. The owner\'s first device is registered automatically on sign-in. To add more tablets, you use pairing codes.\n\nTo register an additional device:\n1. On a device that is already signed in as the owner, open any module and go to Settings & Backup > Backup & Data.\n2. In the Devices panel, tap "Generate pairing code". A code like FARM-7K9X appears.\n3. The code is valid for 30 minutes.\n4. On the new tablet, open the app and sign in. When prompted to set up the device, choose "I have a pairing code" and enter the code.\n5. The new tablet binds to your farm and immediately shows your shared data.\n\nIf you have reached your device limit, the app will not generate a new code until you remove an existing device or upgrade your plan.',
      terms:[
        {t:'Pairing Code',d:'A short code (e.g. FARM-7K9X) the owner generates to register a new device. Expires after 30 minutes.'},
        {t:'Device Limit',d:'The maximum number of devices your plan allows. Starter includes 2, Professional 5, Enterprise 15.'},
        {t:'Registered Device',d:'A tablet that has been bound to your farm and can access shared data.'},
      ]
    },
    {
      title:'Managing & Removing Devices',
      preview:'View your registered devices, see usage against your limit, and remove devices.',
      body:'The Devices panel (Settings & Backup > Backup & Data) shows every device registered to your farm, with its name, platform, and when it was last active. It also shows your usage — for example "2 of 2 devices" — against your plan limit.\n\nTo free up a device slot, tap "Remove" next to a device. That device will need a new pairing code to rejoin. This is useful when a tablet is lost, replaced, or no longer in use.\n\nOnly the farm owner or a manager can register or remove devices. This prevents staff from adding unauthorised tablets.',
      terms:[
        {t:'Device Usage',d:'How many of your allowed device slots are currently in use.'},
        {t:'Remove Device',d:'Revoking a device\'s access, which frees a slot for another tablet.'},
      ]
    },
    {
      title:'Subscription & Plan Limits',
      preview:'How plans, trials, and device entitlements work.',
      body:'Every new farm starts on a free trial. Your plan determines how many devices you can register and which modules are available.\n\nStarter: up to 2 devices, core PoultryOS.\nProfessional: up to 5 devices, adds HatcheryOS and analytics.\nEnterprise: up to 15 devices, all modules including FeedMillOS.\n\nWhen you reach your device limit, you can either remove a device you no longer use, or upgrade your plan to add more capacity. Your data is never lost when a trial ends — it remains safely in your farm account.',
      terms:[
        {t:'Trial',d:'A time-limited free period for new farms to evaluate the app.'},
        {t:'Plan Tier',d:'Starter, Professional, or Enterprise — each with its own device limit and module access.'},
      ]
    },
    {
      title:'Setting Up a New Farm',
      preview:'What happens when you first sign up, and how to get going quickly.',
      body:'When you sign up, a farm account is created for you automatically and you become its owner. The first time you open PoultryOS on a new farm, a short onboarding guide appears to help you get started.\n\nThe onboarding walks you through:\n1. Entering your farm name and location (used on reports).\n2. Adding your first house — a building or pen that will hold birds.\n3. A summary of what to do next.\n\nYou can skip onboarding at any time and set things up later. After onboarding, the typical first steps are:\n- Houses & Batches: add houses, then create your first batch of birds.\n- Daily Log: record mortality, feed, and vaccinations each day.\n- Financials: log costs and sales to track profit per batch.\n\nIf you skipped onboarding and want guidance again, every engine has an empty-state hint, and this Help center has full step-by-step guides for each feature.',
      terms:[
        {t:'Onboarding',d:'A short first-run guide that helps a new farm set up its basics.'},
        {t:'House',d:'A physical building or pen that holds a batch of birds.'},
...
const HELP_TROUBLESHOOTING_SECTION={
  id:'troubleshooting',label:'Troubleshooting & FAQ',icon:'cloud',
  topics:[
    {
      title:'My data isn\'t showing on another device',
      preview:'What to check when two tablets aren\'t showing the same records.',
      body:'PoultrySuite syncs your farm\'s data across all registered devices, but a few things can delay it:\n\n1. Check both devices are signed in to the SAME farm account. Data is shared within one farm, not between separate accounts.\n\n2. Confirm both devices are registered to the farm. Open Settings & Backup > Devices and verify both appear in the list.\n\n3. Check internet connectivity. A device that is offline keeps working, but its new entries only appear on other devices after it reconnects and syncs.\n\n4. Reload the other device. Sync happens when the app loads and as you work; pulling the screen to refresh or reopening the app fetches the latest data.\n\n5. If a device was offline for a while, give it a moment after reconnecting — queued changes upload automatically.',
      terms:[
        {t:'Sync Delay',d:'The short period before an offline device\'s changes appear elsewhere, resolved on reconnect.'},
      ]
    },
    {
      title:'The app shows an old version after an update',
      preview:'How to force the app to load the latest version.',
      body:'PoultrySuite is installed as an app on your device and caches itself so it can run offline. After an update is released, a device may briefly keep showing the cached older version.\n\nTo get the latest version:\n- In a browser: do a hard refresh (press and hold the reload button, or use your browser\'s "reload without cache" option).\n- Installed on a tablet: fully close the app (swipe it away from recent apps) and reopen it. If it persists, remove the app from your home screen and re-add it.\n\nThis only needs to be done once after a major update; routine use does not require it.',
      terms:[]
    },
    {
      title:'I can\'t sign in or forgot my password',
      preview:'Recovering access to your account.',
      body:'If your email and password are not accepted, check for typos and that Caps Lock is off. Passwords are case-sensitive.\n\nIf you have forgotten your password, use the "Forgot password?" link on the sign-in screen to request a reset email. Follow the link in that email to set a new password.\n\nIf you do not receive the email, check your spam folder and confirm you used the email address you registered with. If you are still stuck, contact support (see "Still need help?" at the bottom of this Help center).',
      terms:[]
    },
    {
      title:'Working offline — what to expect',
      preview:'How the app behaves with no internet, and how it catches up.',
      body:'PoultrySuite is built to work offline. With no internet you can still open the app, view your existing data, and record new entries — mortality, feed, eggs, production, everything.\n\nWhat happens behind the scenes: changes you make offline are saved on the device and placed in a queue. The moment the device regains internet, that queue uploads automatically and your changes sync to your other devices.\n\nWhat to keep in mind: another device will not see your offline changes until your device reconnects. If two devices edit the same record while both offline, the most recent save wins once they both reconnect.',
      terms:[
        {t:'Offline Queue',d:'Changes saved on a device while offline, uploaded automatically on reconnect.'},
        {t:'Last-write-wins',d:'When the same record is edited on two devices, the most recent save is kept.'},
      ]
    },
    {
      title:'Frequently asked questions',
      preview:'Quick answers to common questions about the app.',
      body:'Does the app need internet? No for daily recording — it works offline and syncs when reconnected. Internet is needed to share data across devices and to sign in for the first time on a new device.\n\nIs my data backed up? Yes — your data lives in your secure farm account in the cloud, so it survives a lost or damaged device. Sign in on a replacement device and your data is there.\n\nCan staff and the owner use it at the same time? Yes — each registered device works independently and shares the same farm data.\n\nWhat happens when my trial ends? Your data is kept safe. You upgrade to a paid plan to continue full access.\n\nHow do I add another tablet? The owner generates a pairing code (Settings & Backup > Devices) and enters it on the new tablet. See "Accounts & Devices".',
      terms:[]
    },
  ]
};

// Shared "Best Practices" — operational guidance, identical across modules.
const HELP_BESTPRACTICES_SECTION={
  id:'best-practices',label:'Best Practices',icon:'start',
  topics:[
    {
      title:'Record data daily for accurate insights',
      preview:'Why consistent daily entry makes your reports trustworthy.',
      body:'The quality of your dashboards, alerts, and profit-and-loss figures depends entirely on the data you enter. A few habits make a big difference:\n\nRecord mortality, feed, and egg collection every day, ideally at the same time. Gaps make trends and feed-conversion figures unreliable.\n\nEnter data on the device closest to the work — record mortality in the house, not from memory hours later. Because all devices sync, it does not matter which one you use.\n\nUse the Daily Log as your starting point each morning; it surfaces what needs attention (overdue vaccinations, today\'s tasks) in one place.',
      terms:[]
    },
    {
      title:'Assign devices to roles and locations',
      preview:'A simple device-naming scheme keeps a multi-tablet farm organised.',
      body:'When you register devices, give each a clear name that reflects where or how it is used — for example "Layer house tablet", "Office tablet", or "Manager phone".\n\nThis makes the Devices list easy to read, helps you spot an unfamiliar device quickly, and makes it obvious which device to remove if one is lost or replaced.\n\nKeep the owner\'s device for administration (registering devices, viewing financials) and let staff devices focus on daily recording.',
      terms:[]
    },
    {
      title:'Keeping your account secure',
      preview:'Simple steps to protect your farm\'s data.',
      body:'Your data is protected by your login, so account security matters:\n\nUse a strong, unique password that you do not reuse elsewhere.\n\nDo not share your owner login. If staff need access, register their device under the farm rather than handing out your password.\n\nWhen a device is lost, stolen, or no longer used, remove it from the Devices panel immediately — this revokes its access and frees a slot.\n\nReview your registered devices from time to time and remove any you do not recognise.',
      terms:[]
    },
  ]
};
...
const HELP_CONTENT={
  poultry:{
    ...
    overview:'PoultryOS is a cloud-synced, batch-level farm management system. It tracks every aspect of poultry production — from flock intake to sales — across houses, batches, mortality, feed, vaccinations, health events, egg production, and financials. Your data is shared securely across all your farm\'s registered devices and works offline, syncing automatically when you reconnect.',
    sections:[
      {
        ...
        topics:[
          ...
          },
          {
            ...
            body:'Command Center: Live dashboard showing all active batch KPIs, mortality and egg-production charts, vaccination alerts, and totals. Always your starting point.\n\nDaily Log: The fastest way to record today\'s mortality, feed, and overdue vaccinations. You can attach a photo when logging mortality.\n\nHouses & Batches: Register buildings and create flocks. Manage batch status changes (Active, Quarantined, Sold, Closed).\n\nEgg Production: Log daily egg collections, crate counts, and sales for Layer batches.\n\nVaccinations: Schedule doses, apply full breed templates, and mark vaccinations as administered.\n\nFeed Tracking: Log daily feed consumption per batch. Track FCR and total feed cost.\n\nHealth & Mortality: Record disease events and mortality, attach photos, and flag quarantine.\n\nVeterinary & Medical: Treatments, drug inventory, prescriptions, medication schedule, vet consultation notes, a disease library, and a symptom checker.\n\nQuarantine & Biosecurity: Manage quarantine cases, isolation zones, sanitation logs, visitor/vehicle access, outbreak response, and case resolution (which auto-updates mortality, batch counts and financials).\n\nCustomers & Orders: Track buyers and their orders (eggs, broilers, chicks, etc.) with fulfilment status and revenue.\n\nFinancials: Log all costs and revenues per batch. View batch-level P&L.\n\nAudit Log: Every system action is recorded with timestamp, entity, and user.\n\nSettings & Backup: Configure alerts and thresholds, manage devices and device PINs, and export/import your data.',
            terms:[]
          },
        ]
      },
      {...HELP_ACCOUNTS_SECTION},
      {...HELP_BESTPRACTICES_SECTION},
      {
        id:'newfeatures',label:'Health, Quarantine & Sales',icon:'overview',
        topics:[
          {
            title:'Veterinary & Medical Engine',
            preview:'Treatments, drugs, prescriptions, vet notes, disease library and a symptom checker.',
            body:'The Veterinary & Medical engine centralises animal-health management:\n\nTreatments: record a condition, drug, dosage, route, duration and outcome. End dates and withdrawal periods are tracked.\n\nDrug Inventory: track stock, reorder levels, expiry dates and supplier/cost. Low-stock and expiring-soon warnings appear automatically.\n\nPrescriptions: log prescribed medications with dosage, frequency and duration.\n\nMedication Schedule: an automatic view of all active prescriptions and ongoing treatment courses.\n\nVet Notes: record consultations — vet name, clinic, diagnosis, recommendation and follow-up date.\n\nDisease Library: reference information on common poultry diseases (signs, prevention, action).\n\nSymptom Checker: select observed symptoms to see conditions to discuss with your vet. This is informational only and not a diagnosis — always confirm with a qualified veterinarian.',
            terms:[
              {t:'Withdrawal period',d:'The time after treatment before products (eggs/meat) are safe for sale.'},
              {t:'Route',d:'How a drug is given — drinking water, feed, injection, etc.'},
            ]
          },
          {
            title:'Quarantine & Biosecurity Engine',
            preview:'Contain disease: cases, isolation zones, sanitation, access control and resolution.',
            body:'The Quarantine & Biosecurity engine helps you contain disease and protect the farm:\n\nDashboard: active cases, birds quarantined, a risk score and mortality-spike detection.\n\nActive Cases: open a quarantine case against a batch (this sets the batch to Quarantined), recording suspected disease, severity and symptoms.\n\nIsolation Zones: define where quarantined birds are kept, with capacity and status.\n\nAccess Control: log visitors, staff and vehicles with disinfection status, and toggle a farm lockdown.\n\nSanitation: record disinfection activities by area, method and chemical.\n\nOutbreak Response: a lockdown control plus an emergency response checklist.\n\nResolving a case: click "Resolve Case" and record how many birds were Dead, Culled, Returned to the batch alive, or Still in Quarantine. The system then updates other engines automatically — dead birds post to Mortality and reduce the batch count, culled birds reduce the count and (with a value) record a financial loss, and returned birds rejoin the batch. When nothing remains in quarantine the batch returns to Active.',
            terms:[
              {t:'Quarantine case',d:'A tracked disease-containment event for a batch or group of birds.'},
              {t:'Lockdown',d:'A farm-wide flag indicating movement and visitor access should be restricted.'},
              {t:'Resolution',d:'Finalising a case by accounting for every quarantined bird.'},
            ]
          },
          {
            title:'Customers & Orders',
            preview:'Track buyers and sales orders for eggs, birds and other products.',
            body:'The Customers & Orders engine tracks who you sell to and what they order.\n\nCustomers: add buyers with phone, location and notes. Each customer shows their order count.\n\nOrders: create an order against a customer — product (eggs, live broilers, dressed chicken, chicks, spent layers, manure, etc.), quantity, unit price and delivery date. Mark orders Pending or Fulfilled. The tab shows pending count and fulfilled revenue.',
            terms:[]
          },
          {
            title:'Photos on Health & Mortality',
            preview:'Attach photos when recording health events or mortality.',
            body:'When logging a Health Event or Mortality (in Health & Mortality, or mortality in the Daily Log), you can attach photos using the Take Photo button. On a phone or tablet this opens the camera; on a desktop computer it opens the file picker (desktops do not support direct camera capture). Photos are stored with the record and shown as thumbnails — tap one to view it full size.',
            terms:[]
          },
          {
            title:'Device PINs & Locking',
            preview:'Unlock shared devices with a 6-digit PIN instead of the account password.',
            body:'On a shared farm device, the account email and password are entered once to set the device up. After that, each user can have their own 6-digit PIN.\n\nManaging PINs: the farm owner or a manager opens Settings, then the Devices/Backup tab, and uses the Device PINs section to create a 6-digit PIN for each user. PINs are stored only on that device.\n\nLocking: tap the Lock button in the top bar to lock immediately, or the app locks itself after an hour of inactivity. To resume, enter your PIN. If no PIN is set up, or you forget it, use the "Owner login" option to sign in with the account password. PINs are a convenience lock for shared devices, not a replacement for keeping devices physically secure.',
            terms:[
              {t:'Device PIN',d:'A 6-digit code, unique to each user on a device, used to unlock after locking.'},
              {t:'Owner login',d:'Fallback to the account email + password when a PIN is unavailable.'},
            ]
          },
          {
            title:'Setting Up a New Farm (Onboarding)',
            preview:'What a brand-new farm sees on first use.',
            body:'The first time an owner opens PoultryOS on a new farm with no batches, a short onboarding guide appears. It helps set the farm name and location and optionally add the first house, then points to the next steps (create a batch, log daily, track finances). You can skip it at any time. To revisit guidance later, use this Help center.',
            terms:[]
          },
        ]
      },
      ...
      },
      {
        ...
        topics:[
          ...
          {
            title:'Biosecurity & Disease Prevention',
            preview:'Practical steps to prevent disease outbreaks on your farm.',
            body:'Prevention is cheaper than treatment. Good biosecurity reduces mortality and protects your flocks:\n\nControl farm access: limit visitors, provide footbaths with disinfectant at house entrances, and require clean clothing or coveralls.\n\nKeep an all-in/all-out flow where possible: place birds of one age together, and fully clean and rest a house between batches before restocking.\n\nWatch for early warning signs: a sudden drop in feed or water intake, unusual droppings, respiratory sounds, or a spike in daily mortality. Log a health event the moment you notice something — early action limits spread.\n\nUse the mortality alert threshold (Settings) so PoultrySuite warns you when a batch crosses your acceptable rate, before a problem becomes severe.\n\nSeparate sick birds quickly using the quarantine feature, and consult a qualified veterinarian for diagnosis and treatment — the app records events but is not a substitute for veterinary advice.',
            terms:[
              {t:'Biosecurity',d:'Measures that prevent disease from entering or spreading on a farm.'},
              {t:'All-in/all-out',d:'Stocking and clearing a house with one age group at a time, with cleaning in between.'},
            ]
          },
        ]
      },
  ...
  hatchery:{
    ...
    overview:'HatcheryOS manages the complete lifecycle of egg hatching — from egg intake through incubation, candling, hatch output, chick processing, vaccination, inventory, and financials. Each egg batch moves through defined lifecycle stages with full traceability at every step. Data is shared securely across your farm\'s registered devices and works offline, syncing when you reconnect.',
    sections:[
      {
        id:'overview',label:'Module Overview',icon:'overview',
        topics:[
  ...
  feedmill:{
    ...
    overview:'FeedMillOS manages the complete feed manufacturing workflow — from recipe formulation through raw material intake, production batching, quality control, finished inventory, and distribution. Every batch is fully costed and traceable from raw ingredient to farm delivery. Data is shared securely across your farm\'s registered devices and works offline, syncing when you reconnect.',
    sections:[
      {
        id:'overview',label:'Module Overview',icon:'overview',
        topics:[
...
function HelpDocEngine({module}){
  ...
  // Derive a soft tint + border from the accent itself (no hardcoded blue).
  // If accent is a 6-digit hex, append low-opacity alpha; else fall back.
  const isHex6 = typeof accent === 'string' && /^#[0-9a-fA-F]{6}$/.test(accent);
  const accentBg = isHex6 ? accent + '14' : (accent===T.ok?T.okBg:'#EEF2F1');   // ~8% tint
  const accentLine = isHex6 ? accent + '33' : (accent===T.ok?T.okLine:'#D9E1DE'); // ~20% border
  // Deep charcoal for article/title text instead of pure black.
  const charcoal = '#2B2F36';
  // Lighter, softer tone for category labels (they read a bit heavy at 700/charcoal).
  const categoryInk = '#5B6370';
    ...
    return(
      <div style={{display:'flex',flexDirection:'column',gap:0,animation:'fadeUp .18s ease'}}>
        ...
        <div style={{padding:'4px 2px 22px',marginBottom:0,borderBottom:`1px solid ${T.line}`}}>
          ...
          <div style={{fontSize:22,fontWeight:800,color:charcoal,lineHeight:1.2,letterSpacing:-0.5}}>{tp.title}</div>
        </div>

        {/* Article body */}
        <div style={{padding:'24px 2px 24px',marginBottom:tp.terms&&tp.terms.length?0:0}}>
          {String(tp.body||tp.preview||'').split('\n').filter(Boolean).map((para,i)=>{
        ...
        <div style={{marginTop:18,padding:'14px 16px',background:T.bg2,border:`1px solid ${T.line}`,display:'flex',alignItems:'flex-start',gap:11}}>
          ...
          <span style={{fontSize:11,color:T.ink4,lineHeight:1.6}}>Your data syncs securely across your registered devices via <strong style={{color:T.ink3,fontWeight:700}}>your farm account</strong>, and works offline — changes sync when you reconnect.</span>
        </div>
      </div>
    );
  }
  ...
  // ── MAIN HELP VIEW ─────────────────────────────────────
  const totalArticles = cfg.sections.reduce((n,s)=>n+s.topics.length,0);
  const totalTerms = cfg.sections.reduce((n,s)=>n+s.topics.reduce((m,t)=>m+((t.terms&&t.terms.length)||0),0),0);
  return(
    <div style={{display:'flex',flexDirection:'column',gap:0}}>

      {/* Hero panel — bold colored header with embedded search (LabOS style) */}
      <div style={{position:'relative',overflow:'hidden',background:`linear-gradient(135deg, ${accent} 0%, ${accent}E6 55%, ${accent}CC 100%)`,padding:'20px 22px 18px',marginBottom:16}}>
        {/* decorative rings */}
        <div style={{position:'absolute',top:-60,right:-40,width:180,height:180,borderRadius:'50%',border:'1px solid rgba(255,255,255,0.12)'}}/>
        <div style={{position:'absolute',top:-30,right:-10,width:120,height:120,borderRadius:'50%',border:'1px solid rgba(255,255,255,0.10)'}}/>
        <div style={{position:'relative'}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
            <div style={{width:26,height:26,background:'rgba(255,255,255,0.18)',display:'flex',alignItems:'center',justifyContent:'center'}}>
              <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.9" strokeLinecap="square" strokeLinejoin="miter"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><path d="M12 17h.01"/></svg>
            </div>
            <span style={{fontSize:10,fontWeight:700,color:'rgba(255,255,255,0.85)',textTransform:'uppercase',letterSpacing:1.6}}>{cfg.moduleName} · Help Center</span>
          </div>
          <div style={{fontSize:20,fontWeight:800,color:'#fff',letterSpacing:-0.5,lineHeight:1.15,marginBottom:14}}>How can we help you?</div>

          {/* Embedded search */}
          <div style={{position:'relative',maxWidth:560}}>
            <div style={{position:'absolute',left:14,top:'50%',transform:'translateY(-50%)',pointerEvents:'none'}}>
              <svg width={15} height={15} viewBox="0 0 24 24" fill="none" stroke={T.ink4} strokeWidth="1.9" strokeLinecap="square"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
            </div>
            <input
              value={search}
              onChange={e=>{setSearch(e.target.value);setActiveTopic(null);}}
              placeholder={`Search ${cfg.moduleName} documentation…`}
              style={{width:'100%',background:'#fff',border:'none',padding:'11px 40px',fontSize:13.5,color:T.ink,outline:'none',fontFamily:'inherit',boxShadow:'0 4px 14px rgba(0,0,0,0.12)',boxSizing:'border-box'}}
            />
            {isSearching&&(
              <button onClick={()=>setSearch('')}
                style={{position:'absolute',right:12,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',cursor:'pointer',color:T.ink4,width:26,height:26,display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,lineHeight:1,fontFamily:'inherit'}}>×</button>
            )}
          </div>
          ...
          <div style={{display:'flex',gap:18,marginTop:12,flexWrap:'wrap'}}>
            <div style={{display:'flex',alignItems:'baseline',gap:5}}>
              <span style={{fontSize:15,fontWeight:800,color:'#fff'}}>{cfg.sections.length}</span>
              <span style={{fontSize:10.5,color:'rgba(255,255,255,0.8)',fontWeight:500}}>categories</span>
            </div>
            <div style={{display:'flex',alignItems:'baseline',gap:5}}>
              <span style={{fontSize:15,fontWeight:800,color:'#fff'}}>{totalArticles}</span>
              <span style={{fontSize:10.5,color:'rgba(255,255,255,0.8)',fontWeight:500}}>articles</span>
            </div>
            <div style={{display:'flex',alignItems:'baseline',gap:5}}>
              <span style={{fontSize:15,fontWeight:800,color:'#fff'}}>{totalTerms}</span>
              <span style={{fontSize:10.5,color:'rgba(255,255,255,0.8)',fontWeight:500}}>glossary terms</span>
            </div>
          </div>
        </div>
      </div>
              ...
              {Object.entries(resultsBySec).map(([secId,sec])=>(
                <div key={secId} style={{marginBottom:14}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:8,padding:'7px 0 7px 12px',borderLeft:`2px solid ${T.lineMid}`,marginBottom:6}}>
                    ...
                    <span style={{fontSize:11,fontWeight:600,color:categoryInk,textTransform:'uppercase',letterSpacing:.7}}>{sec.label}</span>
      ...
      {/* ── BROWSE STATE — LabOS-style landing ── */}
      {!isSearching&&(
        <>
          {/* Category cards grid */}
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:10,marginBottom:20}} className="au">
            {cfg.sections.map(sec=>{
              const count=sec.topics.length;
              const active=activeSec===sec.id;
              return(
                <button key={sec.id} onClick={()=>{setActiveSec(sec.id);setActiveTopic(null);}}
                  style={{display:'flex',alignItems:'center',gap:11,padding:'12px 14px',background:active?accentBg:T.bg0,border:`1px solid ${active?accent:T.line}`,cursor:'pointer',fontFamily:'inherit',textAlign:'left',transition:'border-color .14s, background .14s'}}
                  ...
                  <div style={{width:30,height:30,background:active?T.bg0:accentBg,border:`1px solid ${accentLine}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                    <HelpSectionIcon id={sec.icon} size={16} color={accent}/>
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:600,color:categoryInk,lineHeight:1.25,letterSpacing:-0.1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{sec.label}</div>
                    <div style={{fontSize:10,fontWeight:600,color:accent,textTransform:'uppercase',letterSpacing:0.6,marginTop:2}}>{count} article{count!==1?'s':''}</div>
                  </div>
                </button>
              );
            })}
          ...
          <div style={{padding:'0 2px 10px',marginBottom:8,display:'flex',alignItems:'center',gap:8,borderBottom:`1px solid ${T.line}`}}>
            <div style={{width:26,height:26,background:accentBg,border:`1px solid ${accentLine}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
              <HelpSectionIcon id={currentSection.icon} size={14} color={accent}/>
            </div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:13,fontWeight:600,color:categoryInk,letterSpacing:-0.2,lineHeight:1.2}}>{currentSection.label}</div>
            </div>
            <div style={{fontSize:10.5,color:T.ink4,letterSpacing:0.2}}>{currentSection.topics.length} article{currentSection.topics.length!==1?'s':''}</div>
          </div>

          {/* Article rows for the selected category */}
          <div style={{display:'flex',flexDirection:'column',gap:0,border:`1px solid ${T.line}`,background:T.bg0,marginBottom:22}} className="au" key={activeSec}>
            {currentSection.topics.map((topic,i)=>(
              <button key={i} onClick={()=>setActiveTopic(topic)}
                style={{display:'flex',alignItems:'center',gap:0,background:T.bg0,border:'none',borderTop:i>0?`1px solid ${T.line}`:'none',cursor:'pointer',fontFamily:'inherit',textAlign:'left',transition:'background .12s',width:'100%',padding:0}}
                onMouseEnter={e=>{e.currentTarget.style.background=T.bg1;}}
                onMouseLeave={e=>{e.currentTarget.style.background=T.bg0;}}>
                {/* Article doc icon */}
                <div style={{width:38,padding:'11px 0',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                  <svg width={15} height={15} viewBox="0 0 24 24" fill="none" stroke={T.ink4} strokeWidth="1.6" strokeLinecap="square" strokeLinejoin="miter"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6M8 13h8M8 17h6"/></svg>
                </div>
                <div style={{flex:1,padding:'10px 14px 10px 0',minWidth:0}}>
                  <div style={{fontSize:13.5,fontWeight:600,color:charcoal,lineHeight:1.3,marginBottom:2,letterSpacing:-0.1}}>{topic.title}</div>
                  <div style={{fontSize:11.5,color:T.ink4,lineHeight:1.4,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{topic.preview}</div>
                </div>
                <div style={{display:'flex',alignItems:'center',justifyContent:'center',padding:'0 14px',flexShrink:0}}>
                  <svg width={13} height={13} viewBox="0 0 24 24" fill="none" stroke={T.ink4} strokeWidth="1.75" strokeLinecap="square" strokeLinejoin="miter"><path d="M9 18l6-6-6-6"/></svg>
                </div>
              </button>
            ))}
          </div>
          ...
          <div style={{marginBottom:8}}>
            <div style={{fontSize:11,fontWeight:700,color:T.ink,textTransform:'uppercase',letterSpacing:0.7,marginBottom:12}}>Still need help?</div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:12}}>
              {[
                {k:'email',label:'Email Support',val:'support@agorox.africa',href:'mailto:support@agorox.africa',
                  svg:<><rect x="3" y="5" width="18" height="14"/><path d="M3 7l9 6 9-6"/></>},
                {k:'phone',label:'Call Us',val:'+234 803 412 9684',href:'tel:+2348034129684',
                ...
                {k:'chat',label:'WhatsApp',val:'Chat with support',href:'https://wa.me/2348034129684',
                ...
                {k:'ticket',label:'Documentation',val:'Browse all guides',href:null,
                  svg:<><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></>},
              ].map(c=>{
                const inner=(
                  <>
                    <div style={{width:28,height:28,background:accentBg,border:`1px solid ${accentLine}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                      <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="1.6" strokeLinecap="square" strokeLinejoin="miter">{c.svg}</svg>
                    </div>
                    <div style={{minWidth:0}}>
                      <div style={{fontSize:11.5,fontWeight:700,color:T.ink}}>{c.label}</div>
                      <div style={{fontSize:10.5,color:T.ink4,lineHeight:1.4,wordBreak:'break-word'}}>{c.val}</div>
                    </div>
                  </>
                );
                const style={display:'flex',flexDirection:'row',alignItems:'center',gap:10,padding:'10px 12px',background:T.bg0,border:`1px solid ${T.line}`,textDecoration:'none',cursor:'pointer',transition:'border-color .14s'};
                return c.href?(
                  <a key={c.k} href={c.href} target={c.href.startsWith('http')?'_blank':undefined} rel="noopener noreferrer"
                    style={style}
                    onMouseEnter={e=>e.currentTarget.style.borderColor=accent}
                    onMouseLeave={e=>e.currentTarget.style.borderColor=T.line}>{inner}</a>
                ):(
                  <div key={c.k} style={style}
                    onMouseEnter={e=>e.currentTarget.style.borderColor=accent}
                    onMouseLeave={e=>e.currentTarget.style.borderColor=T.line}>{inner}</div>
                );
              })}
            </div>
          </div>
      ...
      {/* Footer tip */}
      <div style={{marginTop:14,padding:'10px 14px',background:T.bg2,border:`1px solid ${T.line}`,display:'flex',gap:10,alignItems:'flex-start'}}>
        <svg width={13} height={13} viewBox="0 0 24 24" fill="none" stroke={T.ink4} strokeWidth="1.75" strokeLinecap="square" style={{flexShrink:0,marginTop:1}}><circle cx="12" cy="12" r="9"/><path d="M12 8v4M12 16h.01"/></svg>
        <span style={{fontSize:10.5,color:T.ink4,lineHeight:1.5}}>Your farm data syncs securely across your registered devices. <strong style={{color:T.ink3,fontWeight:700}}>Powered by AgoroX Technologies</strong>.</span>
      </div>
    </div>
  );
}
...
const LEGAL_DOCS={
  eula:{
    ...
    sections:[
      ...
      {heading:'2. Grant of License',body:'Subject to payment of applicable fees (or during an active trial), Licensor grants Licensee a non-exclusive, non-transferable, account-based license to access PoultrySuite Africa for the Licensee\'s internal farm-management operations, on the number of devices permitted by the Licensee\'s plan.\n\nYou may not: (a) sublicense, resell, or share account access outside your organisation; (b) decompile or reverse-engineer the Software; (c) exceed your plan\'s device limit by circumventing device registration; (d) access modules outside your plan tier.'},
      {heading:'3. Plan Tiers and Modules',body:'The Software is offered under Starter, Professional, and Enterprise plans, which determine the number of registered devices permitted and which modules (PoultryOS, HatcheryOS, FeedMillOS) and features are available. Accessing modules or capacity outside your plan is prohibited and constitutes a material breach.'},
      {heading:'4. Subscription, Trial and Renewal',body:'New accounts begin with a time-limited free trial. Paid subscriptions are billed annually, commencing on activation, and renew only upon payment. Upon expiry of a paid term or trial, access may be restricted to read-only or suspended until renewal, but Licensee data is retained in the account and is not deleted as a result of expiry.'},
      {heading:'5. Data Ownership',body:'All operational data entered by Licensee remains the sole property of Licensee. Licensor stores and processes this data only to provide the service (including synchronisation across the Licensee\'s registered devices) and makes no ownership claim to it.'},
      {heading:'6. Data Storage',body:'PoultrySuite Africa stores operational data in a secure cloud account associated with the Licensee\'s farm, enabling synchronisation across the Licensee\'s registered devices. Data is also cached locally on each device to support offline use, and synchronises when connectivity is available.'},
  ...
  privacy:{
    ...
    sections:[
      ...
      {heading:'2. Information We Collect',body:'Profile Information: During registration you provide name, farm name, phone, email, and location. This is stored in your secure farm account and used to operate your account, including authentication and your subscription.\n\nOperational Data: Farm records you create are stored in your farm\'s secure cloud account so they can sync across your registered devices. This data is private to your farm and is not used for any purpose other than providing the service to you.\n\nDevice Information: When you register a device, we store a device identifier, name, and platform to enforce your plan\'s device limit and show you which devices have access.'},
      {heading:'3. How We Use Information',body:'We use your information solely to provide and operate the service: to authenticate you, create and manage your farm account and subscription, synchronise your data across your registered devices, enforce your plan\'s device limit, and provide support. We do not sell your personal information, and we do not share it with third parties except service providers necessary to operate the platform (see Section 5) or where required by law.'},
      {heading:'4. Data Storage and Security',body:'Your profile and operational data are stored in a secure cloud database operated on our behalf by our infrastructure provider, and are cached locally on your registered devices to support offline use. Access is protected by authentication and database-level access controls so that each farm\'s data is isolated and accessible only to that farm\'s authorised users. While we apply reasonable safeguards, no system is perfectly secure; you are encouraged to keep your login credentials confidential.'},
      {heading:'5. Service Providers',body:'We rely on trusted third-party providers to deliver the service: our cloud database and authentication provider (which stores your account and operational data), and Paystack for payment processing. These providers process data only as needed to provide their function. We do not collect or store your card details; all payment data is handled securely by Paystack under their own privacy policy.'},
      {heading:'6. Cookies and Tracking',body:'PoultrySuite Africa stores limited information on your device necessary for the service to function (such as your session and a device identifier). We do not use third-party advertising cookies or tracking pixels, and we do not track you across other websites.'},
      {heading:'7. Your Rights',body:'Consistent with the Nigeria Data Protection Act/Regulation, you have the right to: (a) access the personal information we hold about you; (b) correct inaccurate information; (c) request deletion of your account and associated data; (d) export your data; and (e) object to or restrict certain processing. To exercise these rights, contact our Data Protection Officer below.'},
      {heading:'8. Data Retention',body:'We retain your account and operational data for as long as your account is active. If you close your account or request deletion, we will delete or anonymise your personal data within a reasonable period, except where retention is required by law.'},
...
const PS_SEED={
  ...
  healthLogs:[{id:'HL001',batchId:'BT002',date:'2025-01-25',symptom:'Reduced Feed Intake',severity:'Mild',treatment:'Vitamin B complex in water',status:'Resolved',quarantine:false,notes:'',photos:[]}],
  vetTreatments:[{id:'VT001',batchId:'BT002',date:'2025-01-26',condition:'Suspected Coccidiosis',drug:'Amprolium',dosage:'1g/2L water',route:'Drinking Water',durationDays:5,startDate:'2025-01-26',endDate:'2025-01-31',vet:'Dr. Adewale',outcome:'Ongoing',withdrawalDays:0,notes:'Monitor droppings daily.'}],
  vetDrugs:[{id:'VD001',name:'Amprolium 20%',category:'Anticoccidial',unit:'sachet',quantity:12,reorderLevel:4,expiryDate:'2026-03-01',supplier:'AgroVet Ltd',costPerUnit:1500,notes:''},{id:'VD002',name:'Oxytetracycline',category:'Antibiotic',unit:'bottle',quantity:6,reorderLevel:3,expiryDate:'2025-12-15',supplier:'AgroVet Ltd',costPerUnit:2200,notes:''},{id:'VD003',name:'Vitamin B Complex',category:'Supplement',unit:'sachet',quantity:20,reorderLevel:6,expiryDate:'2026-06-01',supplier:'FarmCare',costPerUnit:600,notes:''}],
  vetPrescriptions:[{id:'VP001',batchId:'BT002',date:'2025-01-26',drug:'Amprolium 20%',dosage:'1g per 2L water',frequency:'Once daily',durationDays:5,prescribedBy:'Dr. Adewale',status:'Active',notes:'For suspected coccidiosis.'}],
  vetConsults:[{id:'VC001',batchId:'BT002',date:'2025-01-26',vetName:'Dr. Adewale Okoro',clinic:'Ibadan Poultry Vet Services',phone:'+234 802 000 1111',reason:'Reduced feed intake, bloody droppings',diagnosis:'Suspected coccidiosis',recommendation:'Start Amprolium 5-day course; improve litter dryness.',followUpDate:'2025-02-02',notes:''}],
  bioCases:[{id:'BC001',title:'Suspected coccidiosis — House B',batchId:'BT002',date:'2025-01-25',severity:'Medium',status:'Active',zoneId:'BZ001',birdsQuarantined:120,suspectedDisease:'Coccidiosis',symptoms:'Bloody droppings, lethargy',notes:'Isolated affected birds pending vet confirmation.',resolution:null,rootCause:'',recommendation:'',resolvedDate:null}],
  bioZones:[{id:'BZ001',name:'Isolation Pen 1',location:'Rear of farm, 50m from main houses',capacity:200,status:'Occupied',notes:'Downwind isolation unit'},{id:'BZ002',name:'Isolation Pen 2',location:'Quarantine block',capacity:150,status:'Available',notes:''}],
  bioSanitation:[{id:'BS001',date:'2025-01-25',area:'House B',method:'Footbath + spray disinfection',chemical:'Virkon S',doneBy:'Farm staff',notes:'After isolation of affected birds'}],
  bioAccess:[{id:'BA001',date:'2025-01-26',type:'Visitor',name:'Feed supplier rep',org:'Oyo Grains Ltd',vehicle:'Van — OYO-234-XY',purpose:'Feed delivery',disinfected:true,notes:''}],
  bioLockdown:false,
  bioCompliance:{footbaths:true,visitorLog:true,rodentControl:true,allInAllOut:false,vaccinationCurrent:true,wasteDisposal:true},
  customers:[{id:'PC001',name:'Mama Tunde Foods',phone:'+234 803 444 5555',location:'Bodija Market, Ibadan',notes:'Buys eggs weekly'},{id:'PC002',name:'Royale Restaurant',phone:'+234 806 777 8888',location:'Ibadan',notes:'Broiler orders monthly'}],
  orders:[{id:'PO001',customerId:'PC001',date:'2025-01-22',product:'Eggs (crates)',qty:50,unitPrice:3500,status:'Fulfilled',deliveryDate:'2025-01-23',notes:''},{id:'PO002',customerId:'PC002',date:'2025-01-28',product:'Live Broilers',qty:80,unitPrice:4500,status:'Pending',deliveryDate:'2025-02-05',notes:'Dressed on request'}],
...
function CommandCenter({state,dispatch,dataMode}){
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      {/* Visual KPI reports */}
      <div style={{display:'grid',gridTemplateColumns:layingBatches.length>0?'1fr 1fr':'1fr',gap:10}}>
        <ChartCard title="Mortality — last 7 days" subtitle="birds lost/day">
          <MiniBarChart data={lastNDaysBuckets(mortalityLogs,'date',m=>m.count,7)} color={T.chartBlue} unit=" birds"/>
        </ChartCard>
        {layingBatches.length>0&&(
          <ChartCard title="Egg Sales — last 7 days" subtitle="crates/day">
            <MiniBarChart data={lastNDaysBuckets(financialLogs.filter(f=>f.category==='Egg Sales'),'date',f=>f.qty||0,7)} color={T.chartBlue} unit=" crates"/>
          </ChartCard>
        )}
      </div>
...
// ── Customers & Orders engine (shared by Hatchery + FeedMill) ──
// `cfg` carries module-specific bits: action prefix, unit label, product list.
// ── Sales & Procurement Engine (shared by all three modules) ──────────
// Two sides feeding Financials with ACTUAL CASH:
//  Income:  Customers → Orders → Payments (each payment posts a Revenue log)
//  Expense: Suppliers → Purchase Orders → Vendor Payments (each posts a Cost log)
// cfg carries module products + procurement items.
function SalesProcurementEngine({state,dispatch,cfg}){
  const customers=state.customers||[];
  const orders=state.orders||[];
  const suppliers=state.suppliers||[];
  const purchaseOrders=state.purchaseOrders||[];
  const [side,setSide]=useState('income'); // income | expense
  const [tab,setTab]=useState('orders');
  const money=(n)=>ngn(n);
  const today=todayStr();
  const custName=(id)=>customers.find(c=>c.id===id)?.name||'—';
  const supName=(id)=>suppliers.find(s=>s.id===id)?.name||'—';
  const paidOf=(rec)=>(rec.payments||[]).reduce((s,p)=>s+(Number(p.amount)||0),0);
  const totalOf=(rec)=>(Number(rec.qty)||0)*(Number(rec.unitPrice)||0);
  const payStatus=(rec)=>{const t=totalOf(rec),p=paidOf(rec);if(p<=0)return'Outstanding';if(p>=t)return'Paid';return'Partial';};
  const unitsFor=(product)=>{const m=/\(([^)]+)\)/.exec(product||'');if(m){const pl=m[1].trim().toLowerCase();return{unit:pl,unitSing:pl.replace(/s$/,'')};}return{unit:cfg.unit,unitSing:cfg.unitSing};};

  // modals
  const [mCust,setMCust]=useState(false);
  const [mOrder,setMOrder]=useState(false);
  const [mPay,setMPay]=useState(null);     // order awaiting payment
  const [mSup,setMSup]=useState(false);
  const [mPO,setMPO]=useState(false);
  const [mVPay,setMVPay]=useState(null);    // PO awaiting vendor payment

  const [cF,setCF]=useState({name:'',phone:'',location:'',notes:''});
  const [oF,setOF]=useState({customerId:'',date:today,product:cfg.products[0]||'',qty:'',unitPrice:'',deliveryDate:'',notes:''});
  const [payF,setPayF]=useState({amount:'',date:today,method:'Cash',notes:''});
  const [sF,setSF]=useState({name:'',material:'',phone:'',location:'',notes:''});
  const [poF,setPOF]=useState({supplierId:'',date:today,item:cfg.procureItems[0]||'',qty:'',unitPrice:'',unit:'kg',expectedDate:'',notes:''});
  const [vpayF,setVPayF]=useState({amount:'',date:today,method:'Bank Transfer',notes:''});
  const formUnits=unitsFor(oF.product);

  // ── savers ──
  const saveCust=()=>{if(!cF.name)return;dispatch({type:'SP_ADD_CUSTOMER',p:{id:uid('CUS'),name:cF.name,phone:cF.phone,location:cF.location,notes:cF.notes}});setCF({name:'',phone:'',location:'',notes:''});setMCust(false);};
  const saveOrder=()=>{if(!oF.customerId||!oF.qty)return;dispatch({type:'SP_ADD_ORDER',p:{id:uid('ORD'),customerId:oF.customerId,date:oF.date,product:oF.product,qty:Number(oF.qty)||0,unitPrice:Number(oF.unitPrice)||0,unit:unitsFor(oF.product).unit,deliveryDate:oF.deliveryDate,notes:oF.notes,payments:[]}});setOF({customerId:'',date:today,product:cfg.products[0]||'',qty:'',unitPrice:'',deliveryDate:'',notes:''});setMOrder(false);};
  const savePay=()=>{if(!mPay||!payF.amount)return;dispatch({type:'SP_ADD_ORDER_PAYMENT',p:{order:mPay,payment:{id:uid('PAY'),date:payF.date,amount:Number(payF.amount)||0,method:payF.method,notes:payF.notes}}});setPayF({amount:'',date:today,method:'Cash',notes:''});setMPay(null);};
  const saveSup=()=>{if(!sF.name)return;dispatch({type:'SP_ADD_SUPPLIER',p:{id:uid('SUP'),name:sF.name,material:sF.material,phone:sF.phone,location:sF.location,notes:sF.notes}});setSF({name:'',material:'',phone:'',location:'',notes:''});setMSup(false);};
  const savePO=()=>{if(!poF.supplierId||!poF.qty)return;dispatch({type:'SP_ADD_PO',p:{id:uid('PO'),supplierId:poF.supplierId,supplierName:supName(poF.supplierId),date:poF.date,item:poF.item,qty:Number(poF.qty)||0,unitPrice:Number(poF.unitPrice)||0,unit:poF.unit,expectedDate:poF.expectedDate,notes:poF.notes,payments:[]}});setPOF({supplierId:'',date:today,item:cfg.procureItems[0]||'',qty:'',unitPrice:'',unit:'kg',expectedDate:'',notes:''});setMPO(false);};
  const saveVPay=()=>{if(!mVPay||!vpayF.amount)return;dispatch({type:'SP_ADD_PO_PAYMENT',p:{po:mVPay,payment:{id:uid('VPY'),date:vpayF.date,amount:Number(vpayF.amount)||0,method:vpayF.method,notes:vpayF.notes}}});setVPayF({amount:'',date:today,method:'Bank Transfer',notes:''});setMVPay(null);};

  // ── totals ──
  const totalReceived=orders.reduce((s,o)=>s+paidOf(o),0);
  const totalInvoiced=orders.reduce((s,o)=>s+totalOf(o),0);
  const outstandingIn=Math.max(0,totalInvoiced-totalReceived);
  const totalPaidOut=purchaseOrders.reduce((s,p)=>s+paidOf(p),0);
  const totalPO=purchaseOrders.reduce((s,p)=>s+totalOf(p),0);
  const outstandingOut=Math.max(0,totalPO-totalPaidOut);

  const sideTabs = side==='income'
    ? [{id:'orders',label:'Orders'},{id:'customers',label:'Customers'},{id:'payments',label:'Payments'}]
    : [{id:'pos',label:'Purchase Orders'},{id:'suppliers',label:'Suppliers'},{id:'vpayments',label:'Vendor Payments'}];
  const switchSide=(s)=>{setSide(s);setTab(s==='income'?'orders':'pos');};

  const statusChip=(st)=>({Paid:{bg:T.okBg,fg:T.ok},Partial:{bg:T.warnBg,fg:T.warn},Outstanding:{bg:T.bg2,fg:T.ink3}}[st]||{bg:T.bg2,fg:T.ink3});

  return(
    <div>
      {/* Income / Expense switch */}
      <div style={{display:'flex',gap:8,marginBottom:14}}>
        <button onClick={()=>switchSide('income')} style={{flex:1,padding:'10px',border:`1px solid ${side==='income'?T.accent:T.line}`,background:side==='income'?T.accentBg:T.bg0,color:side==='income'?T.accentDark:T.ink3,fontWeight:700,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>Sales (Income)</button>
        <button onClick={()=>switchSide('expense')} style={{flex:1,padding:'10px',border:`1px solid ${side==='expense'?T.accent:T.line}`,background:side==='expense'?T.accentBg:T.bg0,color:side==='expense'?T.accentDark:T.ink3,fontWeight:700,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>Procurement (Expense)</button>
      </div>

      {/* summary cards */}
      {side==='income'?(
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))',gap:8,marginBottom:14}}>
          <KPICard label="Received" value={money(totalReceived)} sub="posted to revenue"/>
          <KPICard label="Invoiced" value={money(totalInvoiced)}/>
          <KPICard label="Outstanding" value={money(outstandingIn)} alert={outstandingIn>0}/>
        </div>
      ):(
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))',gap:8,marginBottom:14}}>
          <KPICard label="Paid Out" value={money(totalPaidOut)} sub="posted to cost"/>
          <KPICard label="PO Value" value={money(totalPO)}/>
          <KPICard label="Outstanding" value={money(outstandingOut)} alert={outstandingOut>0}/>
        </div>
      )}

      <TabBar tabs={sideTabs} active={tab} onChange={setTab}/>

      {/* ORDERS */}
      {tab==='orders'&&(<>
        <div style={{marginBottom:12}}><Btn size="sm" onClick={()=>setMOrder(true)} disabled={customers.length===0}>+ New Order</Btn>{customers.length===0&&<span style={{fontSize:11,color:T.ink4,marginLeft:8}}>Add a customer first.</span>}</div>
        {mOrder&&<Modal title="New Sales Order" onClose={()=>setMOrder(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Sel label="Customer *" value={oF.customerId} onChange={v=>setOF(f=>({...f,customerId:v}))} options={[{value:'',label:'Select customer'},...customers.map(c=>({value:c.id,label:c.name}))]}/>
          <Sel label="Product" value={oF.product} onChange={v=>setOF(f=>({...f,product:v}))} options={cfg.products}/>
          <Inp label={`Quantity (${formUnits.unit}) *`} type="number" value={oF.qty} onChange={v=>setOF(f=>({...f,qty:v}))}/>
          <Inp label={`Price per ${formUnits.unitSing} (₦)`} type="number" value={oF.unitPrice} onChange={v=>setOF(f=>({...f,unitPrice:v}))}/>
          <Inp label="Delivery Date" type="date" value={oF.deliveryDate} onChange={v=>setOF(f=>({...f,deliveryDate:v}))}/>
          <div style={{fontSize:12,color:T.ink3}}>Order total: <strong>{money((Number(oF.qty)||0)*(Number(oF.unitPrice)||0))}</strong></div>
          <Btn onClick={saveOrder} full>Create Order</Btn>
        </div></Modal>}
        {orders.length===0?<EmptyState title="No orders yet" sub="Create a sales order, then record payments against it"/>:
          orders.map(o=>{const st=payStatus(o);const chip=statusChip(st);const total=totalOf(o);const paid=paidOf(o);return(<div key={o.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px',marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>{custName(o.customerId)}</div><div style={{fontSize:11,color:T.ink4}}>{o.product} · {fmtDate(o.date)}</div></div>
              <span style={{fontSize:10,padding:'2px 6px',fontWeight:600,background:chip.bg,color:chip.fg}}>{st}</span>
            </div>
            <div style={{fontSize:12,color:T.ink3,marginTop:6}}>{Number(o.qty).toLocaleString()} {o.unit||unitsFor(o.product).unit} × {money(o.unitPrice)} = <strong>{money(total)}</strong></div>
            <div style={{fontSize:11,color:T.ink4,marginTop:3}}>Paid {money(paid)} · Outstanding {money(Math.max(0,total-paid))}</div>
            <div style={{marginTop:8,display:'flex',gap:8}}>
              <Btn size="sm" onClick={()=>{setPayF({amount:String(Math.max(0,total-paid)),date:today,method:'Cash',notes:''});setMPay(o);}}>Record Payment</Btn>
              <Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'SP_DELETE_ORDER',p:o.id})}>Delete</Btn>
            </div>
          </div>);})}
      </>)}

      {/* PAYMENT modal (orders) */}
      {mPay&&<Modal title={`Payment — ${custName(mPay.customerId)}`} onClose={()=>setMPay(null)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
        <Notice type="info" message="Recording a payment posts that amount to Financials as revenue (actual cash received)."/>
        <div style={{fontSize:12,color:T.ink3}}>Order total {money(totalOf(mPay))} · already paid {money(paidOf(mPay))}</div>
        <Inp label="Payment Amount (₦) *" type="number" value={payF.amount} onChange={v=>setPayF(f=>({...f,amount:v}))}/>
        <Sel label="Method" value={payF.method} onChange={v=>setPayF(f=>({...f,method:v}))} options={['Cash','Bank Transfer','POS','Mobile Money','Cheque']}/>
        <Inp label="Date" type="date" value={payF.date} onChange={v=>setPayF(f=>({...f,date:v}))}/>
        <Btn onClick={savePay} full>Record Payment</Btn>
      </div></Modal>}

      {/* CUSTOMERS */}
      {tab==='customers'&&(<>
        <div style={{marginBottom:12}}><Btn size="sm" onClick={()=>setMCust(true)}>+ Add Customer</Btn></div>
        {mCust&&<Modal title="Add Customer" onClose={()=>setMCust(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Inp label="Name *" value={cF.name} onChange={v=>setCF(f=>({...f,name:v}))}/>
          <Inp label="Phone" value={cF.phone} onChange={v=>setCF(f=>({...f,phone:v}))}/>
          <Inp label="Location" value={cF.location} onChange={v=>setCF(f=>({...f,location:v}))}/>
          <Inp label="Notes" value={cF.notes} onChange={v=>setCF(f=>({...f,notes:v}))}/>
          <Btn onClick={saveCust} full>Add Customer</Btn>
        </div></Modal>}
        {customers.length===0?<EmptyState title="No customers" sub="Add the buyers you sell to"/>:
          customers.map(c=>{const co=orders.filter(o=>o.customerId===c.id);return(<div key={c.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px',marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>{c.name}</div><div style={{fontSize:11,color:T.ink4}}>{c.location||'—'} · {co.length} order{co.length!==1?'s':''}</div></div>
              {c.phone&&<a href={`tel:${c.phone}`} style={{fontSize:12,color:T.accent}}>{c.phone}</a>}
            </div>
            {c.notes&&<div style={{fontSize:12,color:T.ink3,marginTop:6}}>{c.notes}</div>}
            <div style={{marginTop:8}}><Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'SP_DELETE_CUSTOMER',p:c.id})}>Delete</Btn></div>
          </div>);})}
      </>)}

      {/* PAYMENTS ledger (income) */}
      {tab==='payments'&&(()=>{
        const all=[];orders.forEach(o=>(o.payments||[]).forEach(p=>all.push({...p,who:custName(o.customerId),orderId:o.id})));
        all.sort((a,b)=>(b.date||'').localeCompare(a.date||''));
        return all.length===0?<EmptyState title="No payments recorded" sub="Payments you record on orders appear here and post to revenue"/>:(
          <div>{all.map(p=>(<div key={p.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'12px 14px',marginBottom:8,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>{money(p.amount)} <span style={{fontSize:11,color:T.ink4,fontWeight:400}}>· {p.method}</span></div><div style={{fontSize:11,color:T.ink4}}>{p.who} · {fmtDate(p.date)}</div></div>
            <Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'SP_DELETE_ORDER_PAYMENT',p:{orderId:p.orderId,paymentId:p.id}})}>Remove</Btn>
          </div>))}</div>
        );
      })()}
      ...
      {/* PURCHASE ORDERS */}
      {tab==='pos'&&(<>
        <div style={{marginBottom:12}}><Btn size="sm" onClick={()=>setMPO(true)} disabled={suppliers.length===0}>+ New Purchase Order</Btn>{suppliers.length===0&&<span style={{fontSize:11,color:T.ink4,marginLeft:8}}>Add a supplier first.</span>}</div>
        {mPO&&<Modal title="New Purchase Order" onClose={()=>setMPO(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Sel label="Supplier *" value={poF.supplierId} onChange={v=>setPOF(f=>({...f,supplierId:v}))} options={[{value:'',label:'Select supplier'},...suppliers.map(s=>({value:s.id,label:s.name}))]}/>
          <Sel label="Item" value={poF.item} onChange={v=>setPOF(f=>({...f,item:v}))} options={cfg.procureItems}/>
          <Inp label="Quantity *" type="number" value={poF.qty} onChange={v=>setPOF(f=>({...f,qty:v}))}/>
          <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:10}}>
            <Inp label="Unit Price (₦)" type="number" value={poF.unitPrice} onChange={v=>setPOF(f=>({...f,unitPrice:v}))}/>
            <Sel label="Unit" value={poF.unit} onChange={v=>setPOF(f=>({...f,unit:v}))} options={['kg','tonne','bag','litre','unit','dose','crate']}/>
          </div>
          <Inp label="Expected Date" type="date" value={poF.expectedDate} onChange={v=>setPOF(f=>({...f,expectedDate:v}))}/>
          <div style={{fontSize:12,color:T.ink3}}>PO total: <strong>{money((Number(poF.qty)||0)*(Number(poF.unitPrice)||0))}</strong></div>
          <Btn onClick={savePO} full>Create Purchase Order</Btn>
        </div></Modal>}
        {purchaseOrders.length===0?<EmptyState title="No purchase orders" sub="Raise a PO to a supplier, then record vendor payments"/>:
          purchaseOrders.map(p=>{const st=payStatus(p);const chip=statusChip(st);const total=totalOf(p);const paid=paidOf(p);return(<div key={p.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px',marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>{supName(p.supplierId)}</div><div style={{fontSize:11,color:T.ink4}}>{p.item} · {fmtDate(p.date)}</div></div>
              <span style={{fontSize:10,padding:'2px 6px',fontWeight:600,background:chip.bg,color:chip.fg}}>{st}</span>
            </div>
            <div style={{fontSize:12,color:T.ink3,marginTop:6}}>{Number(p.qty).toLocaleString()} {p.unit} × {money(p.unitPrice)} = <strong>{money(total)}</strong></div>
            <div style={{fontSize:11,color:T.ink4,marginTop:3}}>Paid {money(paid)} · Outstanding {money(Math.max(0,total-paid))}</div>
            <div style={{marginTop:8,display:'flex',gap:8}}>
              <Btn size="sm" onClick={()=>{setVPayF({amount:String(Math.max(0,total-paid)),date:today,method:'Bank Transfer',notes:''});setMVPay(p);}}>Record Vendor Payment</Btn>
              <Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'SP_DELETE_PO',p:p.id})}>Delete</Btn>
            </div>
          </div>);})}
      </>)}

      {/* VENDOR PAYMENT modal */}
      {mVPay&&<Modal title={`Vendor Payment — ${supName(mVPay.supplierId)}`} onClose={()=>setMVPay(null)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
        <Notice type="info" message="Recording a vendor payment posts that amount to Financials as a cost (actual cash paid)."/>
        <div style={{fontSize:12,color:T.ink3}}>PO total {money(totalOf(mVPay))} · already paid {money(paidOf(mVPay))}</div>
        <Inp label="Payment Amount (₦) *" type="number" value={vpayF.amount} onChange={v=>setVPayF(f=>({...f,amount:v}))}/>
        <Sel label="Method" value={vpayF.method} onChange={v=>setVPayF(f=>({...f,method:v}))} options={['Bank Transfer','Cash','POS','Mobile Money','Cheque']}/>
        <Inp label="Date" type="date" value={vpayF.date} onChange={v=>setVPayF(f=>({...f,date:v}))}/>
        <Btn onClick={saveVPay} full>Record Vendor Payment</Btn>
      </div></Modal>}

      {/* SUPPLIERS */}
      {tab==='suppliers'&&(<>
        <div style={{marginBottom:12}}><Btn size="sm" onClick={()=>setMSup(true)}>+ Add Supplier</Btn></div>
        {mSup&&<Modal title="Add Supplier" onClose={()=>setMSup(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Inp label="Supplier Name *" value={sF.name} onChange={v=>setSF(f=>({...f,name:v}))}/>
          <Inp label="Material / Goods Supplied" value={sF.material} onChange={v=>setSF(f=>({...f,material:v}))}/>
          <Inp label="Phone" value={sF.phone} onChange={v=>setSF(f=>({...f,phone:v}))}/>
          <Inp label="Location" value={sF.location} onChange={v=>setSF(f=>({...f,location:v}))}/>
          <Inp label="Notes" value={sF.notes} onChange={v=>setSF(f=>({...f,notes:v}))}/>
          <Btn onClick={saveSup} full>Add Supplier</Btn>
        </div></Modal>}
        {suppliers.length===0?<EmptyState title="No suppliers" sub="Add vendors you buy from"/>:
          suppliers.map(s=>{const sp=purchaseOrders.filter(p=>p.supplierId===s.id);return(<div key={s.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px',marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>{s.name}</div><div style={{fontSize:11,color:T.ink4}}>{s.material||'—'}{s.location?' · '+s.location:''} · {sp.length} PO{sp.length!==1?'s':''}</div></div>
              {s.phone&&<a href={`tel:${s.phone}`} style={{fontSize:12,color:T.accent}}>{s.phone}</a>}
            </div>
            {s.notes&&<div style={{fontSize:12,color:T.ink3,marginTop:6}}>{s.notes}</div>}
            <div style={{marginTop:8}}><Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'SP_DELETE_SUPPLIER',p:s.id})}>Delete</Btn></div>
          </div>);})}
      </>)}

      {/* VENDOR PAYMENTS ledger */}
      {tab==='vpayments'&&(()=>{
        const all=[];purchaseOrders.forEach(p=>(p.payments||[]).forEach(x=>all.push({...x,who:supName(p.supplierId),poId:p.id})));
        all.sort((a,b)=>(b.date||'').localeCompare(a.date||''));
        return all.length===0?<EmptyState title="No vendor payments" sub="Payments you record on POs appear here and post to cost"/>:(
          <div>{all.map(p=>(<div key={p.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'12px 14px',marginBottom:8,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>{money(p.amount)} <span style={{fontSize:11,color:T.ink4,fontWeight:400}}>· {p.method}</span></div><div style={{fontSize:11,color:T.ink4}}>{p.who} · {fmtDate(p.date)}</div></div>
            <Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'SP_DELETE_PO_PAYMENT',p:{poId:p.poId,paymentId:p.id}})}>Remove</Btn>
          </div>))}</div>
        );
      })()}
    </div>
...
// Camera/photo capture: downsizes to ~900px JPEG so images stay small enough
// to sync as data URLs. Uses the device camera on mobile/tablet via capture.
function PhotoCapture({photos=[],onChange,max=4}){
  ...
  const onPick=async(e)=>{
    ...
    const out=[];
    for(const f of files.slice(0,slots)){const d=await resize(f);if(d)out.push(d);}
    if(out.length)onChange([...photos,...out]);
    if(e.target)e.target.value='';
  };
  return(
    <div>
      <div style={{fontSize:12,fontWeight:600,color:T.ink3,marginBottom:6}}>Photos {photos.length>0&&`(${photos.length}/${max})`}</div>
      <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
        {photos.map((src,i)=>(
          <div key={i} style={{position:'relative'}}>
            <img src={src} alt="" style={{width:60,height:60,objectFit:'cover',border:`1px solid ${T.line}`}}/>
            <button type="button" onClick={()=>onChange(photos.filter((_,j)=>j!==i))} style={{position:'absolute',top:-7,right:-7,width:20,height:20,borderRadius:'50%',border:'none',background:T.ink3,color:'#fff',fontSize:13,lineHeight:1,cursor:'pointer',fontFamily:'inherit'}}>×</button>
          </div>
        ))}
      </div>
      {photos.length<max&&(
        <div style={{display:'flex',gap:8,marginTop:8,flexWrap:'wrap'}}>
          <button type="button" onClick={()=>camRef.current&&camRef.current.click()} style={{display:'flex',alignItems:'center',gap:7,padding:'9px 14px',border:`1px solid ${T.line}`,background:T.bg1,cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:600,color:T.ink2}}>
            <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke={T.ink3} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>
            Take Photo
          </button>
        </div>
      )}
...
// ── Quarantine & Biosecurity Engine ──────────────────────────
function BiosecurityEngine({state,dispatch}){
  const {batches=[],bioCases=[],bioZones=[],bioSanitation=[],bioAccess=[],bioLockdown=false,bioCompliance={},mortalityLogs=[]}=state;
  const [tab,setTab]=useState('dashboard');
  const today=todayStr();
  const batchName=(id)=>batches.find(b=>b.id===id)?.name||'—';
  const zoneName=(id)=>bioZones.find(z=>z.id===id)?.name||'—';
  const money=(n)=>ngn(n);

  // modals
  const [mCase,setMCase]=useState(false);
  const [mZone,setMZone]=useState(false);
  const [mSan,setMSan]=useState(false);
  const [mAcc,setMAcc]=useState(false);
  const [mResolve,setMResolve]=useState(null); // holds the case being resolved

  const [caseF,setCaseF]=useState({title:'',batchId:'',date:today,severity:'Medium',zoneId:'',birdsQuarantined:'',suspectedDisease:'',symptoms:'',notes:''});
  const [zoneF,setZoneF]=useState({name:'',location:'',capacity:'',status:'Available',notes:''});
  const [sanF,setSanF]=useState({date:today,area:'',method:'',chemical:'',doneBy:'',notes:''});
  const [accF,setAccF]=useState({date:today,type:'Visitor',name:'',org:'',vehicle:'',purpose:'',disinfected:false,notes:''});
  const [resF,setResF]=useState({dead:'',returned:'',culled:'',stillQ:'',cullValuePer:'',rootCause:'',recommendation:''});

  const activeCases=bioCases.filter(c=>c.status==='Active');
  const resolvedCases=bioCases.filter(c=>c.status==='Resolved');

  const saveCase=()=>{if(!caseF.title)return;dispatch({type:'BIO_ADD_CASE',p:{id:uid('BC'),title:caseF.title,batchId:caseF.batchId,date:caseF.date,severity:caseF.severity,status:'Active',zoneId:caseF.zoneId,birdsQuarantined:Number(caseF.birdsQuarantined)||0,suspectedDisease:caseF.suspectedDisease,symptoms:caseF.symptoms,notes:caseF.notes,resolution:null,rootCause:'',recommendation:'',resolvedDate:null}});setCaseF({title:'',batchId:'',date:today,severity:'Medium',zoneId:'',birdsQuarantined:'',suspectedDisease:'',symptoms:'',notes:''});setMCase(false);};
  const saveZone=()=>{if(!zoneF.name)return;dispatch({type:'BIO_ADD_ZONE',p:{id:uid('BZ'),name:zoneF.name,location:zoneF.location,capacity:Number(zoneF.capacity)||0,status:zoneF.status,notes:zoneF.notes}});setZoneF({name:'',location:'',capacity:'',status:'Available',notes:''});setMZone(false);};
  const saveSan=()=>{if(!sanF.area)return;dispatch({type:'BIO_ADD_SANITATION',p:{id:uid('BS'),date:sanF.date,area:sanF.area,method:sanF.method,chemical:sanF.chemical,doneBy:sanF.doneBy,notes:sanF.notes}});setSanF({date:today,area:'',method:'',chemical:'',doneBy:'',notes:''});setMSan(false);};
  const saveAcc=()=>{if(!accF.name)return;dispatch({type:'BIO_ADD_ACCESS',p:{id:uid('BA'),date:accF.date,type:accF.type,name:accF.name,org:accF.org,vehicle:accF.vehicle,purpose:accF.purpose,disinfected:accF.disinfected,notes:accF.notes}});setAccF({date:today,type:'Visitor',name:'',org:'',vehicle:'',purpose:'',disinfected:false,notes:''});setMAcc(false);};
  const openResolve=(c)=>{setResF({dead:'',returned:'',culled:'',stillQ:'',cullValuePer:'',rootCause:'',recommendation:''});setMResolve(c);};
  const saveResolve=()=>{if(!mResolve)return;dispatch({type:'BIO_RESOLVE_CASE',p:{caseId:mResolve.id,date:today,resolution:{dead:resF.dead,returned:resF.returned,culled:resF.culled,stillQ:resF.stillQ},cullValuePer:resF.cullValuePer,rootCause:resF.rootCause,recommendation:resF.recommendation}});setMResolve(null);};

  // Mortality spike detection (last 3 days vs prior 7-day average)
  const recentMort=lastNDaysBuckets(mortalityLogs,'date',m=>m.count,3).reduce((s,d)=>s+d.value,0);
  const priorAvg=lastNDaysBuckets(mortalityLogs,'date',m=>m.count,10).slice(0,7).reduce((s,d)=>s+d.value,0)/7;
  const mortSpike=priorAvg>0 && recentMort/3 > priorAvg*1.5;

  // Risk score (simple 0-100): active cases, lockdown, spike, compliance gaps
  const complianceItems=['footbaths','visitorLog','rodentControl','allInAllOut','vaccinationCurrent','wasteDisposal'];
  const complianceScore=complianceItems.filter(k=>bioCompliance[k]).length/complianceItems.length;
  const risk=Math.min(100,Math.round(activeCases.length*20 + (mortSpike?25:0) + (bioLockdown?15:0) + (1-complianceScore)*30));
  const riskLevel=risk>=60?'High':risk>=30?'Medium':'Low';

  const totalQuarantined=bioCases.reduce((s,c)=>s+(c.birdsQuarantined||0),0);
  const totalDead=resolvedCases.reduce((s,c)=>s+(c.resolution?.dead||0),0);
  const totalReturned=resolvedCases.reduce((s,c)=>s+(c.resolution?.returned||0),0);
  const recoveryPct=totalQuarantined>0?Math.round(totalReturned/totalQuarantined*100):0;
  const mortalityPct=totalQuarantined>0?Math.round(totalDead/totalQuarantined*100):0;

  const setCompliance=(k,v)=>dispatch({type:'BIO_SET_COMPLIANCE',p:{...bioCompliance,[k]:v}});

  return(
    <div>
      {/* Lockdown banner */}
      {bioLockdown&&<div style={{marginBottom:12}}><Notice type="warn" message="FARM LOCKDOWN ACTIVE — movement and visitor access restricted. Lift from Outbreak Response when safe."/></div>}

      <TabBar tabs={[
        {id:'dashboard',label:'Dashboard'},
        {id:'cases',label:'Active Cases'},
        {id:'zones',label:'Isolation Zones'},
        {id:'access',label:'Access Control'},
        {id:'sanitation',label:'Sanitation'},
        {id:'outbreak',label:'Outbreak Response'},
      ...
      {tab==='dashboard'&&(<>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:8,marginBottom:14}}>
          <KPICard label="Active Cases" value={activeCases.length} alert={activeCases.length>0}/>
          <KPICard label="Birds Quarantined" value={fmtN(totalQuarantined)}/>
          <KPICard label="Risk Level" value={riskLevel} sub={`score ${risk}/100`} alert={riskLevel==='High'}/>
          <KPICard label="Lockdown" value={bioLockdown?'ON':'Off'} alert={bioLockdown}/>
        </div>
        {mortSpike&&<Notice type="warn" message={`Mortality spike detected — recent daily average (${Math.round(recentMort/3)}) is well above the prior trend. Investigate and consider quarantine.`}/>}
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginTop:12}}>
          <ChartCard title="Mortality — last 7 days" subtitle="spread signal">
            <MiniBarChart data={lastNDaysBuckets(mortalityLogs,'date',m=>m.count,7)} color={T.chartBlue} unit=" birds"/>
          </ChartCard>
          <div style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px'}}>
            <div style={{fontSize:12,fontWeight:700,color:T.ink4,textTransform:'uppercase',letterSpacing:0.7,marginBottom:10}}>Biosecurity Checklist</div>
            <div style={{fontSize:13,color:T.ink}}>{Math.round(complianceScore*100)}% complete</div>
            <div style={{height:6,background:T.bg2,marginTop:8}}><div style={{width:`${complianceScore*100}%`,height:'100%',background:T.chartBlue}}/></div>
            <div style={{fontSize:11,color:T.ink4,marginTop:8}}>{complianceItems.filter(k=>!bioCompliance[k]).length} item(s) outstanding — see Compliance tab.</div>
          </div>
        </div>
      </>)}
      ...
      {/* ACTIVE CASES */}
      {tab==='cases'&&(<>
        <div style={{marginBottom:12}}><Btn size="sm" onClick={()=>setMCase(true)}>+ Open Quarantine Case</Btn></div>
        {mCase&&<Modal title="Open Quarantine Case" onClose={()=>setMCase(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Inp label="Case Title *" value={caseF.title} onChange={v=>setCaseF(f=>({...f,title:v}))} placeholder="e.g. Suspected Newcastle — House A"/>
          <Sel label="Affected Batch" value={caseF.batchId} onChange={v=>setCaseF(f=>({...f,batchId:v}))} options={[{value:'',label:'Select batch (optional)'},...batches.map(b=>({value:b.id,label:b.name}))]}/>
          <Sel label="Severity" value={caseF.severity} onChange={v=>setCaseF(f=>({...f,severity:v}))} options={['Low','Medium','High','Critical']}/>
          <Sel label="Isolation Zone" value={caseF.zoneId} onChange={v=>setCaseF(f=>({...f,zoneId:v}))} options={[{value:'',label:'Assign later'},...bioZones.map(z=>({value:z.id,label:z.name}))]}/>
          <Inp label="Birds Quarantined" type="number" value={caseF.birdsQuarantined} onChange={v=>setCaseF(f=>({...f,birdsQuarantined:v}))}/>
          <Inp label="Suspected Disease" value={caseF.suspectedDisease} onChange={v=>setCaseF(f=>({...f,suspectedDisease:v}))}/>
          <Inp label="Symptoms Observed" value={caseF.symptoms} onChange={v=>setCaseF(f=>({...f,symptoms:v}))}/>
          <Btn onClick={saveCase} full>Open Case (quarantines batch)</Btn>
        </div></Modal>}
        {activeCases.length===0?<EmptyState title="No active quarantine cases" sub="Open a case when you isolate birds for disease"/>:
          activeCases.map(c=>(<div key={c.id} style={{background:T.bg0,border:`1px solid ${T.line}`,borderLeft:`3px solid ${c.severity==='Critical'||c.severity==='High'?T.warn:T.chartBlue}`,padding:'14px 16px',marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div><div style={{fontSize:13,fontWeight:700,color:T.ink}}>{c.title}</div><div style={{fontSize:11,color:T.ink4}}>{c.batchId?batchName(c.batchId)+' · ':''}{fmtDate(c.date)}{c.zoneId?' · '+zoneName(c.zoneId):''}</div></div>
              <span style={{fontSize:10,padding:'2px 6px',fontWeight:600,background:T.bg2,color:T.ink3}}>{c.severity}</span>
            </div>
            {c.suspectedDisease&&<div style={{fontSize:12,color:T.ink3,marginTop:6}}>Suspected: {c.suspectedDisease}{c.birdsQuarantined?` · ${c.birdsQuarantined} birds`:''}</div>}
            {c.symptoms&&<div style={{fontSize:12,color:T.ink4,marginTop:3}}>{c.symptoms}</div>}
            <div style={{marginTop:8,display:'flex',gap:8}}>
              <Btn size="sm" onClick={()=>openResolve(c)}>Resolve Case</Btn>
              <Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'BIO_DELETE_CASE',p:c.id})}>Delete</Btn>
            </div>
          </div>))}
      </>)}
      ...
      {/* ISOLATION ZONES */}
      {tab==='zones'&&(<>
        <div style={{marginBottom:12}}><Btn size="sm" onClick={()=>setMZone(true)}>+ Add Isolation Zone</Btn></div>
        {mZone&&<Modal title="Add Isolation Zone" onClose={()=>setMZone(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Inp label="Zone Name *" value={zoneF.name} onChange={v=>setZoneF(f=>({...f,name:v}))}/>
          <Inp label="Location" value={zoneF.location} onChange={v=>setZoneF(f=>({...f,location:v}))}/>
          <Inp label="Capacity (birds)" type="number" value={zoneF.capacity} onChange={v=>setZoneF(f=>({...f,capacity:v}))}/>
          <Sel label="Status" value={zoneF.status} onChange={v=>setZoneF(f=>({...f,status:v}))} options={['Available','Occupied','Closed for Cleaning']}/>
          <Btn onClick={saveZone} full>Add Zone</Btn>
        </div></Modal>}
        {bioZones.length===0?<EmptyState title="No isolation zones" sub="Define where quarantined birds are kept"/>:
          bioZones.map(z=>(<div key={z.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px',marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>{z.name}</div><div style={{fontSize:11,color:T.ink4}}>{z.location||'—'}{z.capacity?` · cap ${z.capacity}`:''}</div></div>
              <span style={{fontSize:10,padding:'2px 6px',fontWeight:600,background:z.status==='Available'?T.okBg:T.bg2,color:z.status==='Available'?T.ok:T.ink3}}>{z.status}</span>
            </div>
            <div style={{marginTop:8}}><Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'BIO_DELETE_ZONE',p:z.id})}>Delete</Btn></div>
          </div>))}
      </>)}
      ...
      {tab==='access'&&(<>
        <div style={{marginBottom:12,display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
          <Btn size="sm" onClick={()=>setMAcc(true)}>+ Log Visitor / Vehicle</Btn>
          <Btn size="sm" variant={bioLockdown?'primary':'secondary'} onClick={()=>dispatch({type:'BIO_SET_LOCKDOWN',p:!bioLockdown})}>{bioLockdown?'Lift Lockdown':'Enable Farm Lockdown'}</Btn>
        </div>
        {mAcc&&<Modal title="Access Log Entry" onClose={()=>setMAcc(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Sel label="Type" value={accF.type} onChange={v=>setAccF(f=>({...f,type:v}))} options={['Visitor','Staff','Vehicle','Delivery','Vet']}/>
          <Inp label="Name *" value={accF.name} onChange={v=>setAccF(f=>({...f,name:v}))}/>
          <Inp label="Organisation" value={accF.org} onChange={v=>setAccF(f=>({...f,org:v}))}/>
          <Inp label="Vehicle (reg / description)" value={accF.vehicle} onChange={v=>setAccF(f=>({...f,vehicle:v}))}/>
          <Inp label="Purpose" value={accF.purpose} onChange={v=>setAccF(f=>({...f,purpose:v}))}/>
          <div style={{display:'flex',gap:8,alignItems:'center',cursor:'pointer'}} onClick={()=>setAccF(f=>({...f,disinfected:!f.disinfected}))}><div style={{width:18,height:18,border:`2px solid ${accF.disinfected?T.ok:T.line}`,background:accF.disinfected?T.ok:T.bg0,display:'flex',alignItems:'center',justifyContent:'center'}}>{accF.disinfected&&<IcoCheck size={10} color="#fff"/>}</div><span style={{fontSize:13,color:T.ink}}>Disinfection / footbath completed</span></div>
          <Btn onClick={saveAcc} full>Log Entry</Btn>
        </div></Modal>}
        {bioAccess.length===0?<EmptyState title="No access logs" sub="Record visitors and vehicles for traceability"/>:
          bioAccess.map(a=>(<div key={a.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'12px 14px',marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>{a.name} <span style={{fontSize:10,color:T.ink4,fontWeight:500}}>· {a.type}</span></div><div style={{fontSize:11,color:T.ink4}}>{a.org||''}{a.vehicle?' · '+a.vehicle:''} · {fmtDate(a.date)}</div></div>
              <span style={{fontSize:10,padding:'2px 6px',fontWeight:600,background:a.disinfected?T.okBg:T.warnBg,color:a.disinfected?T.ok:T.warn}}>{a.disinfected?'Disinfected':'Not disinfected'}</span>
            </div>
            {a.purpose&&<div style={{fontSize:12,color:T.ink3,marginTop:5}}>{a.purpose}</div>}
            <div style={{marginTop:6}}><Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'BIO_DELETE_ACCESS',p:a.id})}>Delete</Btn></div>
          </div>))}
      </>)}

      {/* SANITATION */}
      {tab==='sanitation'&&(<>
        <div style={{marginBottom:12}}><Btn size="sm" onClick={()=>setMSan(true)}>+ Log Sanitation</Btn></div>
        {mSan&&<Modal title="Sanitation / Disinfection Log" onClose={()=>setMSan(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Inp label="Area / House *" value={sanF.area} onChange={v=>setSanF(f=>({...f,area:v}))}/>
          <Inp label="Method" value={sanF.method} onChange={v=>setSanF(f=>({...f,method:v}))} placeholder="e.g. Footbath, full spray"/>
          <Inp label="Chemical / Disinfectant" value={sanF.chemical} onChange={v=>setSanF(f=>({...f,chemical:v}))}/>
          <Inp label="Done By" value={sanF.doneBy} onChange={v=>setSanF(f=>({...f,doneBy:v}))}/>
          <Inp label="Date" type="date" value={sanF.date} onChange={v=>setSanF(f=>({...f,date:v}))}/>
          <Btn onClick={saveSan} full>Log Sanitation</Btn>
        </div></Modal>}
        {bioSanitation.length===0?<EmptyState title="No sanitation logs" sub="Record cleaning and disinfection activities"/>:
          bioSanitation.map(sl=>(<div key={sl.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'12px 14px',marginBottom:8}}>
            <div style={{fontSize:13,fontWeight:600,color:T.ink}}>{sl.area}</div>
            <div style={{fontSize:11,color:T.ink4,marginTop:3}}>{sl.method}{sl.chemical?' · '+sl.chemical:''} · {fmtDate(sl.date)}{sl.doneBy?' · '+sl.doneBy:''}</div>
            <div style={{marginTop:6}}><Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'BIO_DELETE_SANITATION',p:sl.id})}>Delete</Btn></div>
          </div>))}
      </>)}

      {/* OUTBREAK RESPONSE / EMERGENCY */}
      {tab==='outbreak'&&(
        <div>
          <div style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'16px',marginBottom:12}}>
            <div style={{fontSize:13,fontWeight:700,color:T.ink,marginBottom:6}}>Farm Lockdown</div>
            <div style={{fontSize:12,color:T.ink3,lineHeight:1.6,marginBottom:12}}>Enable during a confirmed or suspected outbreak to signal that movement, sales, and visitor access should be restricted. This flags a banner across the engine and is recorded in the audit log.</div>
            <Btn variant={bioLockdown?'primary':'secondary'} onClick={()=>dispatch({type:'BIO_SET_LOCKDOWN',p:!bioLockdown})}>{bioLockdown?'Lift Farm Lockdown':'Enable Farm Lockdown'}</Btn>
          </div>
          <div style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'16px'}}>
            <div style={{fontSize:13,fontWeight:700,color:T.ink,marginBottom:8}}>Emergency Response Checklist</div>
            <ol style={{fontSize:12.5,color:T.ink3,lineHeight:1.9,paddingLeft:18,margin:0}}>
              <li>Isolate affected birds in an isolation zone immediately.</li>
              <li>Open a quarantine case (Active Cases tab) and record symptoms.</li>
              <li>Stop all bird/egg movement off the farm; enable lockdown.</li>
              <li>Disinfect affected houses and equipment (log in Sanitation).</li>
              <li>Contact your veterinarian (Veterinary engine → Vet Notes).</li>
              <li>Restrict visitor and vehicle access; log all entries.</li>
              <li>Submit samples for lab testing where available.</li>
              <li>Notify authorities if a notifiable disease is suspected.</li>
            </ol>
          </div>
        </div>
      )}

      {/* Resolve modal — rendered globally so it works from Active Cases */}
      {mResolve&&<Modal title={`Resolve: ${mResolve.title}`} onClose={()=>setMResolve(null)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
        <Notice type="info" message="Dead birds post to Mortality; culled birds reduce batch count and (with a value) record a financial loss; returned birds rejoin the batch alive."/>
        {mResolve.birdsQuarantined>0&&<div style={{fontSize:12,color:T.ink3}}>{mResolve.birdsQuarantined} birds in this case. Account for each below.</div>}
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
          <Inp label="# Dead" type="number" value={resF.dead} onChange={v=>setResF(f=>({...f,dead:v}))}/>
          <Inp label="# Culled" type="number" value={resF.culled} onChange={v=>setResF(f=>({...f,culled:v}))}/>
          <Inp label="# Returned to Batch (alive)" type="number" value={resF.returned} onChange={v=>setResF(f=>({...f,returned:v}))}/>
          <Inp label="# Still in Quarantine" type="number" value={resF.stillQ} onChange={v=>setResF(f=>({...f,stillQ:v}))}/>
        </div>
        <Inp label="Cull value per bird (₦, optional)" type="number" value={resF.cullValuePer} onChange={v=>setResF(f=>({...f,cullValuePer:v}))}/>
        <Inp label="Root Cause (optional)" value={resF.rootCause} onChange={v=>setResF(f=>({...f,rootCause:v}))}/>
        <Inp label="Preventive Recommendation (optional)" value={resF.recommendation} onChange={v=>setResF(f=>({...f,recommendation:v}))}/>
        <Btn onClick={saveResolve} full>Finalize Resolution</Btn>
      </div></Modal>}
      ...
      {tab==='analytics'&&(
        <div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))',gap:8,marginBottom:14}}>
            <KPICard label="Total Cases" value={bioCases.length}/>
            <KPICard label="Active" value={activeCases.length} alert={activeCases.length>0}/>
            <KPICard label="Recovery %" value={recoveryPct+'%'} sub={`${totalReturned} returned`}/>
            <KPICard label="Mortality %" value={mortalityPct+'%'} sub={`${totalDead} dead`} alert={mortalityPct>30}/>
          </div>
          <ChartCard title="Mortality — last 14 days" subtitle="containment view">
            <MiniBarChart data={lastNDaysBuckets(mortalityLogs,'date',m=>m.count,14)} color={T.chartBlue} unit=" birds"/>
          </ChartCard>
          {resolvedCases.length>0&&<div style={{marginTop:14}}>
            <div style={{fontSize:12,fontWeight:700,color:T.ink4,textTransform:'uppercase',letterSpacing:0.7,marginBottom:10}}>Resolved Cases</div>
            {resolvedCases.map(c=>(<div key={c.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px',marginBottom:8}}>
              <div style={{fontSize:13,fontWeight:600,color:T.ink}}>{c.title}</div>
              <div style={{fontSize:11,color:T.ink4}}>{c.batchId?batchName(c.batchId)+' · ':''}resolved {c.resolvedDate?fmtDate(c.resolvedDate):''}</div>
              {c.resolution&&<div style={{fontSize:12,color:T.ink3,marginTop:6}}>Dead {c.resolution.dead||0} · Culled {c.resolution.culled||0} · Returned alive {c.resolution.returned||0}</div>}
              {c.rootCause&&<div style={{fontSize:12,color:T.ink4,marginTop:4}}><strong>Cause:</strong> {c.rootCause}</div>}
              {c.recommendation&&<div style={{fontSize:12,color:T.ink4,marginTop:3}}><strong>Prevention:</strong> {c.recommendation}</div>}
            </div>))}
          </div>}
          <div style={{marginTop:12,fontSize:11,color:T.ink4,lineHeight:1.6}}>Case outcomes feed Mortality, Inventory and Financial engines automatically on resolution. Full activity is recorded in the Audit Log.</div>
        </div>
      )}
    </div>
  );
...
// ── Veterinary & Medical Engine ──────────────────────────────
function VetEngine({state,dispatch}){
  const {batches=[],vetTreatments=[],vetDrugs=[],vetPrescriptions=[],vetConsults=[]}=state;
  const [tab,setTab]=useState('treatments');
  const activeBatches=batches.filter(b=>['Active','Quarantined','Closed','Sold'].includes(b.status));
  const batchName=(id)=>batches.find(b=>b.id===id)?.name||'—';
  const today=todayStr();

  // modals
  const [mTreat,setMTreat]=useState(false);
  const [mDrug,setMDrug]=useState(false);
  const [mPresc,setMPresc]=useState(false);
  const [mConsult,setMConsult]=useState(false);
  const [tF,setTF]=useState({batchId:'',date:today,condition:'',drug:'',dosage:'',route:'Drinking Water',durationDays:'',startDate:today,withdrawalDays:'',vet:'',outcome:'Ongoing',notes:''});
  const [dF,setDF]=useState({name:'',category:'Antibiotic',unit:'bottle',quantity:'',reorderLevel:'',expiryDate:'',supplier:'',costPerUnit:'',notes:''});
  const [pF,setPF]=useState({batchId:'',date:today,drug:'',dosage:'',frequency:'Once daily',durationDays:'',prescribedBy:'',status:'Active',notes:''});
  const [cF,setCF]=useState({batchId:'',date:today,vetName:'',clinic:'',phone:'',reason:'',diagnosis:'',recommendation:'',followUpDate:'',notes:''});

  const saveTreat=()=>{if(!tF.condition)return;const dur=Number(tF.durationDays)||0;const end=dur?addDays(tF.startDate,dur):'';dispatch({type:'VET_ADD_TREATMENT',p:{id:uid('VT'),batchId:tF.batchId,date:tF.date,condition:tF.condition,drug:tF.drug,dosage:tF.dosage,route:tF.route,durationDays:dur,startDate:tF.startDate,endDate:end,withdrawalDays:Number(tF.withdrawalDays)||0,vet:tF.vet,outcome:tF.outcome,notes:tF.notes}});setTF({batchId:'',date:today,condition:'',drug:'',dosage:'',route:'Drinking Water',durationDays:'',startDate:today,withdrawalDays:'',vet:'',outcome:'Ongoing',notes:''});setMTreat(false);};
  const saveDrug=()=>{if(!dF.name)return;dispatch({type:'VET_ADD_DRUG',p:{id:uid('VD'),name:dF.name,category:dF.category,unit:dF.unit,quantity:Number(dF.quantity)||0,reorderLevel:Number(dF.reorderLevel)||0,expiryDate:dF.expiryDate,supplier:dF.supplier,costPerUnit:Number(dF.costPerUnit)||0,notes:dF.notes}});setDF({name:'',category:'Antibiotic',unit:'bottle',quantity:'',reorderLevel:'',expiryDate:'',supplier:'',costPerUnit:'',notes:''});setMDrug(false);};
  const savePresc=()=>{if(!pF.drug)return;dispatch({type:'VET_ADD_PRESCRIPTION',p:{id:uid('VP'),batchId:pF.batchId,date:pF.date,drug:pF.drug,dosage:pF.dosage,frequency:pF.frequency,durationDays:Number(pF.durationDays)||0,prescribedBy:pF.prescribedBy,status:pF.status,notes:pF.notes}});setPF({batchId:'',date:today,drug:'',dosage:'',frequency:'Once daily',durationDays:'',prescribedBy:'',status:'Active',notes:''});setMPresc(false);};
  const saveConsult=()=>{if(!cF.vetName)return;dispatch({type:'VET_ADD_CONSULT',p:{id:uid('VC'),batchId:cF.batchId,date:cF.date,vetName:cF.vetName,clinic:cF.clinic,phone:cF.phone,reason:cF.reason,diagnosis:cF.diagnosis,recommendation:cF.recommendation,followUpDate:cF.followUpDate,notes:cF.notes}});setCF({batchId:'',date:today,vetName:'',clinic:'',phone:'',reason:'',diagnosis:'',recommendation:'',followUpDate:'',notes:''});setMConsult(false);};

  const lowStock=vetDrugs.filter(d=>d.reorderLevel>0&&d.quantity<=d.reorderLevel);
  const expiringSoon=vetDrugs.filter(d=>d.expiryDate&&daysDiff(today,d.expiryDate)<=30&&d.expiryDate>=today);

  return(
    <div>
      <TabBar tabs={[
        {id:'treatments',label:'Treatments'},
        {id:'drugs',label:'Drug Inventory'},
        {id:'prescriptions',label:'Prescriptions'},
        {id:'schedule',label:'Med Schedule'},
        {id:'consults',label:'Vet Notes'},
      ...
      {/* TREATMENTS */}
      {tab==='treatments'&&(<>
        <div style={{marginBottom:12}}><Btn size="sm" onClick={()=>setMTreat(true)}>+ Record Treatment</Btn></div>
        {mTreat&&<Modal title="Record Treatment" onClose={()=>setMTreat(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Sel label="Batch" value={tF.batchId} onChange={v=>setTF(f=>({...f,batchId:v}))} options={[{value:'',label:'Select Batch (optional)'},...activeBatches.map(b=>({value:b.id,label:b.name}))]}/>
          <Inp label="Condition / Diagnosis *" value={tF.condition} onChange={v=>setTF(f=>({...f,condition:v}))}/>
          <Inp label="Drug / Medication" value={tF.drug} onChange={v=>setTF(f=>({...f,drug:v}))}/>
          <Inp label="Dosage" value={tF.dosage} onChange={v=>setTF(f=>({...f,dosage:v}))}/>
          <Sel label="Route" value={tF.route} onChange={v=>setTF(f=>({...f,route:v}))} options={['Drinking Water','Feed','Injection','Spray','Eye Drop','Topical','Oral']}/>
          <Inp label="Start Date" type="date" value={tF.startDate} onChange={v=>setTF(f=>({...f,startDate:v}))}/>
          <Inp label="Duration (days)" type="number" value={tF.durationDays} onChange={v=>setTF(f=>({...f,durationDays:v}))}/>
          <Inp label="Withdrawal Period (days)" type="number" value={tF.withdrawalDays} onChange={v=>setTF(f=>({...f,withdrawalDays:v}))}/>
          <Inp label="Attending Vet" value={tF.vet} onChange={v=>setTF(f=>({...f,vet:v}))}/>
          <Sel label="Outcome" value={tF.outcome} onChange={v=>setTF(f=>({...f,outcome:v}))} options={['Ongoing','Recovered','Partial','No Response','Deceased']}/>
          <Btn onClick={saveTreat} full>Save Treatment</Btn>
        </div></Modal>}
        {vetTreatments.length===0?<EmptyState title="No treatments recorded" sub="Record drug treatments and outcomes"/>:
          vetTreatments.map(t=>(<div key={t.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px',marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>{t.condition}</div><div style={{fontSize:11,color:T.ink4}}>{t.batchId?batchName(t.batchId)+' · ':''}{fmtDate(t.date)}</div></div>
              <span style={{fontSize:10,padding:'2px 6px',fontWeight:600,background:{Recovered:T.okBg,Ongoing:T.warnBg,Deceased:T.errBg}[t.outcome]||T.bg2,color:{Recovered:T.ok,Ongoing:T.warn,Deceased:T.err}[t.outcome]||T.ink3}}>{t.outcome}</span>
            </div>
            {t.drug&&<div style={{fontSize:12,color:T.ink3,marginTop:6}}>{t.drug} · {t.dosage} · {t.route}</div>}
            {t.durationDays>0&&<div style={{fontSize:11,color:T.ink4,marginTop:3}}>{t.durationDays} days ({fmtDate(t.startDate)}{t.endDate?' → '+fmtDate(t.endDate):''}){t.withdrawalDays>0?` · ${t.withdrawalDays}d withdrawal`:''}</div>}
            {t.vet&&<div style={{fontSize:11,color:T.ink4,marginTop:3}}>Vet: {t.vet}</div>}
            <div style={{marginTop:8,display:'flex',gap:8}}>
              {t.outcome==='Ongoing'&&<Btn size="sm" variant="secondary" onClick={()=>dispatch({type:'VET_UPDATE_TREATMENT',p:{...t,outcome:'Recovered'}})}>Mark Recovered</Btn>}
              <Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'VET_DELETE_TREATMENT',p:t.id})}>Delete</Btn>
            </div>
          </div>))}
      </>)}
      ...
      {/* DRUG INVENTORY */}
      {tab==='drugs'&&(<>
        {(lowStock.length>0||expiringSoon.length>0)&&<div style={{marginBottom:12}}>
          {lowStock.length>0&&<Notice type="warn" message={`${lowStock.length} drug(s) at/below reorder level: ${lowStock.map(d=>d.name).join(', ')}`}/>}
          {expiringSoon.length>0&&<Notice type="warn" message={`${expiringSoon.length} drug(s) expiring within 30 days.`}/>}
        </div>}
        <div style={{marginBottom:12}}><Btn size="sm" onClick={()=>setMDrug(true)}>+ Add Drug</Btn></div>
        {mDrug&&<Modal title="Add Drug to Inventory" onClose={()=>setMDrug(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Inp label="Drug Name *" value={dF.name} onChange={v=>setDF(f=>({...f,name:v}))}/>
          <Sel label="Category" value={dF.category} onChange={v=>setDF(f=>({...f,category:v}))} options={['Antibiotic','Anticoccidial','Antiviral','Dewormer','Vaccine','Supplement','Disinfectant','Other']}/>
          <Sel label="Unit" value={dF.unit} onChange={v=>setDF(f=>({...f,unit:v}))} options={['bottle','sachet','vial','box','kg','litre','pack']}/>
          <Inp label="Quantity in Stock" type="number" value={dF.quantity} onChange={v=>setDF(f=>({...f,quantity:v}))}/>
          <Inp label="Reorder Level" type="number" value={dF.reorderLevel} onChange={v=>setDF(f=>({...f,reorderLevel:v}))}/>
          <Inp label="Expiry Date" type="date" value={dF.expiryDate} onChange={v=>setDF(f=>({...f,expiryDate:v}))}/>
          <Inp label="Supplier" value={dF.supplier} onChange={v=>setDF(f=>({...f,supplier:v}))}/>
          <Inp label="Cost per Unit (₦)" type="number" value={dF.costPerUnit} onChange={v=>setDF(f=>({...f,costPerUnit:v}))}/>
          <Btn onClick={saveDrug} full>Add Drug</Btn>
        </div></Modal>}
        {vetDrugs.length===0?<EmptyState title="No drugs in inventory" sub="Track stock, expiry and reorder levels"/>:
          <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
            <thead><tr style={{background:T.bg2}}>{['Drug','Category','Stock','Expiry','Action'].map(h=><th key={h} style={{padding:'8px 10px',textAlign:'left',fontWeight:600,color:T.ink4,fontSize:11}}>{h}</th>)}</tr></thead>
            <tbody>{vetDrugs.map((d,i)=>{const low=d.reorderLevel>0&&d.quantity<=d.reorderLevel;const exp=d.expiryDate&&d.expiryDate<today;return(<tr key={d.id} style={{background:i%2===0?T.bg0:T.bg1,borderBottom:`1px solid ${T.line}`}}>
              <td style={{padding:'9px 10px',color:T.ink,fontWeight:500}}>{d.name}</td>
              <td style={{padding:'9px 10px',color:T.ink3}}>{d.category}</td>
              <td style={{padding:'9px 10px',color:low?T.err:T.ink}} className="mono">{d.quantity} {d.unit}{low?' ⚠':''}</td>
              <td style={{padding:'9px 10px',color:exp?T.err:T.ink3}}>{d.expiryDate?fmtDate(d.expiryDate):'—'}{exp?' (expired)':''}</td>
              <td style={{padding:'9px 10px'}}><Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'VET_DELETE_DRUG',p:d.id})}>Delete</Btn></td>
            </tr>);})}
            </tbody>
          </table></div>}
      </>)}
      ...
      {/* PRESCRIPTIONS */}
      {tab==='prescriptions'&&(<>
        <div style={{marginBottom:12}}><Btn size="sm" onClick={()=>setMPresc(true)}>+ Add Prescription</Btn></div>
        {mPresc&&<Modal title="Add Prescription" onClose={()=>setMPresc(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Sel label="Batch" value={pF.batchId} onChange={v=>setPF(f=>({...f,batchId:v}))} options={[{value:'',label:'Select Batch (optional)'},...activeBatches.map(b=>({value:b.id,label:b.name}))]}/>
          <Inp label="Drug *" value={pF.drug} onChange={v=>setPF(f=>({...f,drug:v}))}/>
          <Inp label="Dosage" value={pF.dosage} onChange={v=>setPF(f=>({...f,dosage:v}))}/>
          <Sel label="Frequency" value={pF.frequency} onChange={v=>setPF(f=>({...f,frequency:v}))} options={['Once daily','Twice daily','Three times daily','Every other day','As needed']}/>
          <Inp label="Duration (days)" type="number" value={pF.durationDays} onChange={v=>setPF(f=>({...f,durationDays:v}))}/>
          <Inp label="Prescribed By" value={pF.prescribedBy} onChange={v=>setPF(f=>({...f,prescribedBy:v}))}/>
          <Btn onClick={savePresc} full>Save Prescription</Btn>
        </div></Modal>}
        {vetPrescriptions.length===0?<EmptyState title="No prescriptions" sub="Track prescribed medications"/>:
          vetPrescriptions.map(p=>(<div key={p.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px',marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>{p.drug}</div><div style={{fontSize:11,color:T.ink4}}>{p.batchId?batchName(p.batchId)+' · ':''}{fmtDate(p.date)}</div></div>
              <span style={{fontSize:10,padding:'2px 6px',fontWeight:600,background:p.status==='Active'?T.okBg:T.bg2,color:p.status==='Active'?T.ok:T.ink3}}>{p.status}</span>
            </div>
            <div style={{fontSize:12,color:T.ink3,marginTop:6}}>{p.dosage} · {p.frequency}{p.durationDays?` · ${p.durationDays} days`:''}</div>
            {p.prescribedBy&&<div style={{fontSize:11,color:T.ink4,marginTop:3}}>By: {p.prescribedBy}</div>}
            <div style={{marginTop:8,display:'flex',gap:8}}>
              {p.status==='Active'&&<Btn size="sm" variant="secondary" onClick={()=>dispatch({type:'VET_UPDATE_PRESCRIPTION',p:{...p,status:'Completed'}})}>Mark Completed</Btn>}
              <Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'VET_DELETE_PRESCRIPTION',p:p.id})}>Delete</Btn>
            </div>
          </div>))}
      </>)}
      ...
      {/* MEDICATION SCHEDULE */}
      {tab==='schedule'&&(()=>{
        const active=vetPrescriptions.filter(p=>p.status==='Active');
        const treatOngoing=vetTreatments.filter(t=>t.outcome==='Ongoing');
        return(<div>
          <div style={{fontSize:12,color:T.ink4,marginBottom:12,lineHeight:1.5}}>Active medications and ongoing treatment courses across all batches.</div>
          {active.length===0&&treatOngoing.length===0?<EmptyState title="Nothing scheduled" sub="Active prescriptions and ongoing treatments appear here"/>:
            <div style={{display:'flex',flexDirection:'column',gap:8}}>
              {active.map(p=>(<div key={p.id} style={{background:T.bg0,border:`1px solid ${T.line}`,borderLeft:`3px solid ${T.accent}`,padding:'12px 14px'}}>
                <div style={{fontSize:13,fontWeight:600,color:T.ink}}>{p.drug}</div>
                <div style={{fontSize:11,color:T.ink4,marginTop:3}}>{p.frequency} · {p.dosage}{p.batchId?' · '+batchName(p.batchId):''}</div>
              </div>))}
              {treatOngoing.map(t=>(<div key={t.id} style={{background:T.bg0,border:`1px solid ${T.line}`,borderLeft:`3px solid ${T.warn}`,padding:'12px 14px'}}>
                <div style={{fontSize:13,fontWeight:600,color:T.ink}}>{t.drug||t.condition}</div>
                <div style={{fontSize:11,color:T.ink4,marginTop:3}}>Treatment course{t.endDate?` ends ${fmtDate(t.endDate)}`:''}{t.batchId?' · '+batchName(t.batchId):''}</div>
              </div>))}
            </div>}
        </div>);
      })()}
      ...
      {tab==='consults'&&(<>
        <div style={{marginBottom:12}}><Btn size="sm" onClick={()=>setMConsult(true)}>+ Add Consultation Note</Btn></div>
        {mConsult&&<Modal title="Vet Consultation Note" onClose={()=>setMConsult(false)}><div style={{display:'flex',flexDirection:'column',gap:12}}>
          <Sel label="Batch" value={cF.batchId} onChange={v=>setCF(f=>({...f,batchId:v}))} options={[{value:'',label:'Select Batch (optional)'},...activeBatches.map(b=>({value:b.id,label:b.name}))]}/>
          <Inp label="Vet Name *" value={cF.vetName} onChange={v=>setCF(f=>({...f,vetName:v}))}/>
          <Inp label="Clinic" value={cF.clinic} onChange={v=>setCF(f=>({...f,clinic:v}))}/>
          <Inp label="Phone" value={cF.phone} onChange={v=>setCF(f=>({...f,phone:v}))}/>
          <Inp label="Reason for Consultation" value={cF.reason} onChange={v=>setCF(f=>({...f,reason:v}))}/>
          <Inp label="Diagnosis" value={cF.diagnosis} onChange={v=>setCF(f=>({...f,diagnosis:v}))}/>
          <Inp label="Recommendation" value={cF.recommendation} onChange={v=>setCF(f=>({...f,recommendation:v}))}/>
          <Inp label="Follow-up Date" type="date" value={cF.followUpDate} onChange={v=>setCF(f=>({...f,followUpDate:v}))}/>
          <Btn onClick={saveConsult} full>Save Note</Btn>
        </div></Modal>}
        {vetConsults.length===0?<EmptyState title="No consultation notes" sub="Record vet visits and advice"/>:
          vetConsults.map(c=>(<div key={c.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px',marginBottom:8}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div><div style={{fontSize:13,fontWeight:600,color:T.ink}}>{c.vetName}</div><div style={{fontSize:11,color:T.ink4}}>{c.clinic?c.clinic+' · ':''}{fmtDate(c.date)}</div></div>
              {c.followUpDate&&<span style={{fontSize:10,padding:'2px 6px',background:T.accentBg,color:T.accentDark,fontWeight:600}}>Follow-up {fmtDate(c.followUpDate)}</span>}
            </div>
            {c.reason&&<div style={{fontSize:12,color:T.ink3,marginTop:6}}><strong>Reason:</strong> {c.reason}</div>}
            {c.diagnosis&&<div style={{fontSize:12,color:T.ink3,marginTop:3}}><strong>Diagnosis:</strong> {c.diagnosis}</div>}
            {c.recommendation&&<div style={{fontSize:12,color:T.ink3,marginTop:3}}><strong>Advice:</strong> {c.recommendation}</div>}
            {c.phone&&<a href={`tel:${c.phone}`} style={{fontSize:11,color:T.accent,marginTop:6,display:'inline-block'}}>{c.phone}</a>}
            <div style={{marginTop:8}}><Btn size="sm" variant="ghost" onClick={()=>dispatch({type:'VET_DELETE_CONSULT',p:c.id})}>Delete</Btn></div>
          </div>))}
      </>)}
      ...
      {tab==='library'&&(
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          <div style={{fontSize:12,color:T.ink4,marginBottom:4,lineHeight:1.5}}>Common poultry diseases reference. Always confirm with a qualified veterinarian before treating.</div>
          {POULTRY_DISEASES.map(d=>(<details key={d.name} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'12px 14px'}}>
            <summary style={{fontSize:13,fontWeight:600,color:T.ink,cursor:'pointer'}}>{d.name} <span style={{fontSize:10,fontWeight:500,color:T.ink4}}>· {d.type}</span></summary>
            <div style={{fontSize:12,color:T.ink3,marginTop:8,lineHeight:1.6}}>
              <div><strong>Signs:</strong> {d.signs}</div>
              <div style={{marginTop:4}}><strong>Prevention:</strong> {d.prevention}</div>
              <div style={{marginTop:4}}><strong>Action:</strong> {d.action}</div>
            </div>
          </details>))}
        </div>
      )}
      ...
      {/* SYMPTOM CHECKER */}
      {tab==='checker'&&<SymptomChecker/>}
    </div>
  );
}

// Reference data + symptom checker (informational only, not a diagnosis)
const POULTRY_DISEASES=[
  {name:'Newcastle Disease',type:'Viral',signs:'Respiratory distress, twisted neck, greenish diarrhoea, drop in egg production, sudden deaths.',prevention:'Vaccinate (HB1, LaSota) on schedule; strict biosecurity.',action:'No cure; isolate, support flock, consult a vet. Vaccinate healthy birds.'},
  {name:'Coccidiosis',type:'Parasitic',signs:'Bloody or watery droppings, ruffled feathers, reduced feed intake, weakness.',prevention:'Dry litter, coccidiostat in feed, avoid overcrowding.',action:'Anticoccidials (e.g. amprolium) in water; improve litter dryness.'},
  {name:'Fowl Pox',type:'Viral',signs:'Wart-like lesions on comb/wattles, scabs, sometimes mouth lesions.',prevention:'Wing-web vaccination; mosquito control.',action:'Supportive care; isolate affected birds; vaccinate the flock.'},
  {name:'Infectious Bronchitis',type:'Viral',signs:'Coughing, sneezing, nasal discharge, misshapen eggs, drop in lay.',prevention:'Vaccination; good ventilation.',action:'Supportive care, warmth, electrolytes; consult a vet.'},
  {name:'Gumboro (IBD)',type:'Viral',signs:'Watery diarrhoea, vent pecking, depression, sudden mortality in young birds.',prevention:'Vaccinate chicks on schedule; disinfect housing.',action:'No specific treatment; supportive care, electrolytes, biosecurity.'},
  {name:'Fowl Cholera',type:'Bacterial',signs:'Swollen wattles, lameness, greenish diarrhoea, sudden death in older birds.',prevention:'Sanitation, rodent control, vaccination in endemic areas.',action:'Antibiotics under vet guidance; remove dead birds promptly.'},
  {name:'CRD (Mycoplasma)',type:'Bacterial',signs:'Chronic coughing, rales, nasal discharge, reduced growth and lay.',prevention:'Buy from clean stock, good ventilation, reduce stress.',action:'Antibiotics (e.g. tylosin) under vet guidance.'},
];

function SymptomChecker(){
  const [sel,setSel]=useState([]);
  const SYMPTOMS=['Respiratory (coughing/sneezing)','Twisted neck / paralysis','Bloody droppings','Watery/green diarrhoea','Skin lesions / scabs','Drop in egg production','Sudden deaths','Reduced feed intake','Swollen head/wattles'];
  const toggle=(s)=>setSel(p=>p.includes(s)?p.filter(x=>x!==s):[...p,s]);
  const matches=POULTRY_DISEASES.map(d=>{
    let score=0;const t=(d.signs+' '+d.name).toLowerCase();
    sel.forEach(s=>{const k=s.split('(')[0].toLowerCase().trim().split(' ')[0];if(t.includes(k)||(s.includes('Respiratory')&&t.includes('cough'))||(s.includes('Bloody')&&t.includes('bloody'))||(s.includes('Twisted')&&t.includes('twisted'))||(s.includes('egg')&&t.includes('egg'))||(s.includes('death')&&t.includes('death'))||(s.includes('lesion')&&t.includes('lesion')))score++;});
    return {d,score};
  }).filter(m=>m.score>0).sort((a,b)=>b.score-a.score);
  return(
    <div>
      <Notice type="info" message="This is an informational guide only — not a diagnosis. Always consult a qualified veterinarian."/>
      <div style={{fontSize:13,fontWeight:600,color:T.ink,margin:'14px 0 8px'}}>Select observed symptoms:</div>
      <div style={{display:'flex',flexWrap:'wrap',gap:8,marginBottom:16}}>
        {SYMPTOMS.map(s=>(<button key={s} type="button" onClick={()=>toggle(s)} style={{fontSize:12,padding:'7px 11px',border:`1px solid ${sel.includes(s)?T.accent:T.line}`,background:sel.includes(s)?T.accentBg:T.bg0,color:sel.includes(s)?T.accentDark:T.ink3,cursor:'pointer',fontFamily:'inherit',fontWeight:sel.includes(s)?600:400}}>{s}</button>))}
      </div>
      {sel.length>0&&(matches.length>0?(
        <div>
          <div style={{fontSize:13,fontWeight:600,color:T.ink,marginBottom:8}}>Possible conditions to discuss with your vet:</div>
          {matches.map(({d,score})=>(<div key={d.name} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'12px 14px',marginBottom:8}}>
            <div style={{fontSize:13,fontWeight:600,color:T.ink}}>{d.name} <span style={{fontSize:10,color:T.ink4}}>· {d.type} · {score} match{score>1?'es':''}</span></div>
            <div style={{fontSize:12,color:T.ink3,marginTop:5,lineHeight:1.5}}><strong>Action:</strong> {d.action}</div>
          </div>))}
        </div>
      ):<EmptyState title="No clear match" sub="Consult a veterinarian for proper diagnosis"/>)}
    </div>
  );
}
...
function HealthEngine({state,dispatch,pendingAction,onActionConsumed}){
  ...
  const [hForm,setHForm]=useState({batchId:'',date:todayStr(),symptom:'',severity:'Mild',treatment:'',quarantine:false,notes:'',photos:[]});
  const [mForm,setMForm]=useState({batchId:'',date:todayStr(),count:'',cause:'Natural',notes:'',photos:[]});
  ...
  const saveHealth=()=>{if(!hForm.batchId||!hForm.symptom)return;dispatch({type:'LOG_HEALTH',p:{id:uid('HL'),batchId:hForm.batchId,date:hForm.date,symptom:hForm.symptom,severity:hForm.severity,treatment:hForm.treatment,status:'Open',quarantine:hForm.quarantine,notes:hForm.notes,photos:hForm.photos||[]}});setHForm({batchId:'',date:todayStr(),symptom:'',severity:'Mild',treatment:'',quarantine:false,notes:'',photos:[]});setShowAddHealth(false);};
  const saveMort=()=>{if(!mForm.batchId||!mForm.count)return;dispatch({type:'LOG_MORT',p:{id:uid('M'),batchId:mForm.batchId,date:mForm.date,count:Number(mForm.count),cause:mForm.cause,notes:mForm.notes,photos:mForm.photos||[]}});setMForm({batchId:'',date:todayStr(),count:'',cause:'Natural',notes:'',photos:[]});setShowAddMort(false);};
  return(
    <div>
      ...
      {tab==='health'&&(<>
        ...
        healthLogs.map((h,i)=><div key={h.id} style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'14px 16px',marginBottom:8}}>
          ...
          {h.photos&&h.photos.length>0&&<div style={{display:'flex',gap:6,marginTop:8,flexWrap:'wrap'}}>{h.photos.map((src,pi)=><img key={pi} src={src} alt="" onClick={()=>{try{const w=window.open();if(w)w.document.write('<img src="'+src+'" style="max-width:100%"/>');}catch(_){}} } style={{width:54,height:54,objectFit:'cover',border:`1px solid ${T.line}`,cursor:'pointer'}}/>)}</div>}
      ...
      {tab==='mortality'&&(<>
        ...
        <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
          ...
          <tbody>{[...mortalityLogs].sort((a,b)=>b.date.localeCompare(a.date)).map((m,i)=><tr key={m.id} style={{background:i%2===0?T.bg0:T.bg1,borderBottom:`1px solid ${T.line}`}}>
            ...
            <td style={{padding:'9px 10px',color:T.ink}} className="mono">{m.count}</td>
            <td style={{padding:'9px 10px',color:T.ink3}}>{m.cause}</td>
            <td style={{padding:'9px 10px'}}>{(()=>{const ph=(m.photos&&m.photos.length)?m.photos:(m.photo?[m.photo]:[]);return ph.length>0?<div style={{display:'flex',gap:4}}>{ph.map((src,pi)=><img key={pi} src={src} alt="" onClick={()=>{try{const w=window.open();if(w)w.document.write('<img src="'+src+'" style="max-width:100%"/>');}catch(_){}}} style={{width:32,height:32,objectFit:'cover',border:`1px solid ${T.line}`,cursor:'pointer'}}/>)}</div>:<span style={{color:T.ink4}}>—</span>;})()}</td>
          </tr>)}
          </tbody>
        </table></div>}
      </>)}
...
function DailyLogEngine({state,dispatch}){
  ...
  const logMort=()=>{if(!mForm.batchId||!mForm.count)return;dispatch({type:'LOG_MORT',p:{id:uid('M'),batchId:mForm.batchId,date:today,count:Number(mForm.count),cause:mForm.cause,notes:'',photos:mPhoto?[mPhoto.data]:[]}});setMForm({batchId:'',count:'',cause:'Natural'});setMPhoto(null);};
...
function SettingsEngine({state,dispatch,dataMode,onSwitchToLive,onRestoreTraining,license,activeUser,onUpdateLicense}){
  ...
  return(
    <div>
      ...
      {tab==='backup'&&(<>
        <div style={{marginBottom:16}}><DeviceManager/></div>
        <div style={{marginBottom:16}}><DevicePinManager/></div>
        <Notice type="info" message="Your data syncs across your registered devices and works offline."/>
        <div style={{background:T.bg0,border:`1px solid ${T.line}`,padding:'16px 18px'}}>
...
function PoultryModule({navSignal,capacity,license,tier,activeUser,onUpdateLicense,initialSeed,dataMode,onSwitchToLive,onRestoreTraining}){
  ...
  // ── Cross-device batch sync (proven pattern; extends to other types later) ──
  // On mount: load batches from Supabase (shared across the farm's devices).
  // On change: push each batch up so other devices see it. Last-write-wins.
  // Fully offline-tolerant: failed writes queue and flush on reconnect.
  const __syncFarmId = (typeof window!=='undefined' && window.__psaActiveFarmId) || null;
  const __batchSyncReady = useRef(false);
  const __lastSynced = useRef({});
  useEffect(()=>{
    let cancelled=false;
    (async()=>{
      if(!__syncFarmId) { __batchSyncReady.current=true; return; }
      try{
        const mod = await import('../sync/batchSync.js');
        const { batches, offline } = await mod.loadBatches(__syncFarmId);
        if(cancelled) return;
        if(batches && batches.length){
          // Replace local batches with the shared farm set
          dispatch({type:'__SYNC_SET_BATCHES', p:batches});
          batches.forEach(b=>{ __lastSynced.current[b.id]=JSON.stringify(b); });
        }
      }catch(_){}
      __batchSyncReady.current=true;
    })();
    return ()=>{cancelled=true;};
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[__syncFarmId]);

  // Push changed batches up to Supabase
  useEffect(()=>{
    if(!__syncFarmId || !__batchSyncReady.current) return;
    let cancelled=false;
    (async()=>{
      try{
        const mod = await import('../sync/batchSync.js');
        for(const b of ps.batches){
          const sig=JSON.stringify(b);
          if(__lastSynced.current[b.id]!==sig){
            await mod.saveBatch(b, __syncFarmId);
            if(cancelled) return;
            __lastSynced.current[b.id]=sig;
          }
        }
      }catch(_){}
    })();
    return ()=>{cancelled=true;};
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[ps.batches, __syncFarmId]);

  // ── Generic collection sync for the rest of PoultryOS ──
  // houses, mortalityLogs, feedLogs, vaccinations, healthLogs, financialLogs
  const __collSig = useRef({}); // { collection: { id: signature } }
  const __collReady = useRef(false);
  const PS_SYNC_COLLECTIONS = ['houses','mortalityLogs','feedLogs','vaccinations','healthLogs','vetTreatments','vetDrugs','vetPrescriptions','vetConsults','bioCases','bioZones','bioSanitation','bioAccess','customers','orders','suppliers','purchaseOrders','financialLogs'];

  // Load all collections on mount
  useEffect(()=>{
    let cancelled=false;
    (async()=>{
      if(!__syncFarmId){ __collReady.current=true; return; }
      try{
        const mod = await import('../sync/collectionSync.js');
        for(const coll of PS_SYNC_COLLECTIONS){
          const { records } = await mod.loadCollection(__syncFarmId, coll);
          if(cancelled) return;
          if(records && records.length){
            dispatch({type:'__SYNC_SET_COLLECTION', collection:coll, p:records});
            __collSig.current[coll]={};
            records.forEach(r=>{ if(r&&r.id) __collSig.current[coll][r.id]=JSON.stringify(r); });
          }
        }
      }catch(_){}
      __collReady.current=true;
    })();
    return ()=>{cancelled=true;};
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[__syncFarmId]);

  // Push changed records for each collection
  useEffect(()=>{
    if(!__syncFarmId || !__collReady.current) return;
    let cancelled=false;
    (async()=>{
      try{
        const mod = await import('../sync/collectionSync.js');
        for(const coll of PS_SYNC_COLLECTIONS){
          if(!__collSig.current[coll]) __collSig.current[coll]={};
          const records = ps[coll] || [];
          await mod.syncCollectionDiff(__syncFarmId, coll, records, __collSig.current[coll]);
          if(cancelled) return;
        }
      }catch(_){}
    })();
    return ()=>{cancelled=true;};
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[ps.houses, ps.mortalityLogs, ps.feedLogs, ps.vaccinations, ps.healthLogs, ps.vetTreatments, ps.vetDrugs, ps.vetPrescriptions, ps.vetConsults, ps.bioCases, ps.bioZones, ps.bioSanitation, ps.bioAccess, ps.customers, ps.orders, ps.suppliers, ps.purchaseOrders, ps.financialLogs, __syncFarmId]);
  ...
  const renderEngine=(id)=>{
    ...
    else if(id==='vet')     content=<VetEngine            state={ps} dispatch={dispatch}/>;
    else if(id==='bio')     content=<BiosecurityEngine    state={ps} dispatch={dispatch}/>;
    else if(id==='orders')  content=<SalesProcurementEngine state={ps} dispatch={dispatch} cfg={{unit:'units',unitSing:'unit',products:['Eggs (crates)','Live Broilers','Dressed Chicken','Point-of-lay Pullets','Day-old Chicks','Spent Layers','Manure (bags)'],procureItems:['Day-old Chicks','Feed','Drugs & Vaccines','Equipment','Packaging','Litter/Bedding','Other']}}/>;
  ...
  // First-run onboarding: show once for a brand-new farm (no batches yet) that
  // hasn't dismissed it. Owner/super-admin only — staff shouldn't be onboarded.
  const [showOnboard,setShowOnboard]=useState(false);
  useEffect(()=>{
    try{
      const done=localStorage.getItem('psa::onboarded');
      const isOwnerLike=role==='Owner / Director'||role==='farm_owner'||role==='super_admin'||/owner|director|admin/i.test(role||'');
      if(!done && isOwnerLike && dataMode!=='demo' && (ps.batches||[]).length===0){
        setShowOnboard(true);
      }
    }catch(_){}
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return(
    <div style={{display:'grid',gridTemplateColumns:'minmax(220px,260px) 1fr',gap:0,minHeight:500,background:T.bg0,border:`1px solid ${T.line}`}}>
      {showOnboard&&<OnboardingFlow state={ps} dispatch={dispatch} onClose={()=>setShowOnboard(false)}/>}
...
function HKPI({label,value,unit,sub,alert,green}){
  ...
  const str=String(value||'');const fs=str.length>12?13:str.length>8?16:18;
  return(<div style={{background:bg,border:`1px solid ${bord}`,padding:'13px 15px'}}><div style={{fontSize:10,fontWeight:600,color:T.ink4,textTransform:'uppercase',letterSpacing:.7,marginBottom:4}}>{label}</div><div className="mono" style={{fontSize:fs,fontWeight:700,color:c,lineHeight:1,wordBreak:'break-word'}}>{value}{unit&&<span style={{fontSize:11,color:T.ink4,marginLeft:3,fontWeight:400}}>{unit}</span>}</div>{sub&&<div style={{fontSize:11,color:T.ink4,marginTop:2}}>{sub}</div>}</div>);
}
...
function HCmdCenter({state,dataMode}){
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      {/* Visual KPI reports */}
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
        <ChartCard title="Hatch Output — recent" subtitle="chicks/batch">
          <MiniBarChart data={(hatchRecords||[]).slice(0,7).reverse().map((h,i)=>{const eb=eggBatches.find(e=>e.id===h.eggBatchId);return{label:eb?String(eb.batchNo).slice(-4):'#'+(i+1),value:h.totalHatched||0};})} color={T.chartBlue} unit=" chicks"/>
        </ChartCard>
        <ChartCard title="Fertility Trend — recent candling" subtitle="% fertile">
          <MiniLineChart data={(candlingRecords||[]).slice(0,8).reverse().map((c,i)=>({label:'#'+(i+1),value:c.totalCandled>0?Math.round(c.fertile/c.totalCandled*100):0}))} color={T.chartBlue} unit="%"/>
        </ChartCard>
      </div>
...
function HCandling({state,dispatch,pendingAction,onActionConsumed}){
  ...
  const [showForm,setShowForm]=useState(false);
  const [f,setF]=useState({eggBatchId:'',candleDate:hToday(),totalCandled:'',fertile:'',infertile:'',earlyDead:'',lateDead:'',candledBy:'',notes:'',photos:[]});
  const fld=(k,v)=>setF(p=>({...p,[k]:v}));
  ...
  const save=()=>{
    ...
    dispatch({type:'ADD_CANDLE',p:{id:huid('CR'),eggBatchId:f.eggBatchId,candleDate:f.candleDate,totalCandled:tot,fertile,infertile:inf,earlyDead:ed,lateDead:ld,candledBy:f.candledBy,notes:f.notes,photos:f.photos||[]}});
    setF({eggBatchId:'',candleDate:hToday(),totalCandled:'',fertile:'',infertile:'',earlyDead:'',lateDead:'',candledBy:'',notes:'',photos:[]});setShowForm(false);
  };
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      {candlingRecords.length===0?<HEmpty icon="candle" title="No candling records" sub="Record candling to assess fertility per batch"/>:
        candlingRecords.map(cr=>{
          ...
          return(<div key={cr.id} style={{background:fertAlert?T.errBg:'#fff',border:`1px solid ${fertAlert?T.errLine:T.line}`,padding:'14px 16px'}}>
            ...
            {cr.photos&&cr.photos.length>0&&<div style={{display:'flex',gap:6,marginTop:10,flexWrap:'wrap'}}>{cr.photos.map((src,pi)=><img key={pi} src={src} alt="" onClick={()=>{try{const w=window.open();if(w)w.document.write('<img src="'+src+'" style="max-width:100%"/>');}catch(_){}}} style={{width:54,height:54,objectFit:'cover',border:`1px solid ${T.line}`,cursor:'pointer'}}/>)}</div>}
          </div>);
        })
      }
      {showForm&&(<HModal title="Record Candling Results" onClose={()=>setShowForm(false)}>
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          ...
          <PhotoCapture photos={f.photos} onChange={ph=>fld('photos',ph)}/>
...
const HS_SEED={
  ...
  customers:[{id:'HC001',name:'Sunrise Poultry Farm',phone:'+234 803 111 2222',location:'Oyo',notes:'Buys layers monthly'},{id:'HC002',name:'GreenFeather Farms',phone:'+234 805 333 4444',location:'Ogun',notes:''}],
  orders:[{id:'HO001',customerId:'HC001',date:'2025-02-08',product:'Day-old Layer Chicks',qty:2000,unitPrice:500,status:'Fulfilled',deliveryDate:'2025-02-10',notes:''},{id:'HO002',customerId:'HC002',date:'2025-02-12',product:'Day-old Broiler Chicks',qty:1500,unitPrice:480,status:'Pending',deliveryDate:'2025-02-20',notes:'Deposit paid'}],
...
function HatcheryModule({navSignal,capacity,license,activeUser,onUpdateLicense,dataMode,onSwitchToLive,onRestoreTraining,initialSeed}){
  ...
  // ── Cross-device sync for HatcheryOS (same proven pattern as PoultryOS) ──
  ...
  useEffect(()=>{
    let cancelled=false;
    (async()=>{
      if(!__hsFarmId){__hsReady.current=true;return;}
      try{
        const mod=await import('../sync/collectionSync.js');
        for(const coll of HS_SYNC_COLLECTIONS){
          const {records}=await mod.loadCollection(__hsFarmId, 'hs_'+coll);
          if(cancelled)return;
          if(records&&records.length){
            dispatch({type:'__SYNC_SET_COLLECTION', collection:coll, p:records});
            __hsSig.current[coll]={};
            records.forEach(r=>{if(r&&r.id)__hsSig.current[coll][r.id]=JSON.stringify(r);});
          }
        }
      }catch(_){}
      __hsReady.current=true;
    })();
    return ()=>{cancelled=true;};
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[__hsFarmId]);
  useEffect(()=>{
    if(!__hsFarmId||!__hsReady.current)return;
    let cancelled=false;
    (async()=>{
      try{
        const mod=await import('../sync/collectionSync.js');
        for(const coll of HS_SYNC_COLLECTIONS){
          if(!__hsSig.current[coll])__hsSig.current[coll]={};
          await mod.syncCollectionDiff(__hsFarmId, 'hs_'+coll, hs[coll]||[], __hsSig.current[coll]);
          if(cancelled)return;
        }
      }catch(_){}
    })();
...
const FM_SEED={
  ...
  customers:[{id:'FC001',name:'Sunrise Farm',phone:'+234 803 111 2222',location:'Oyo',notes:'Weekly broiler starter'},{id:'FC002',name:'Hilltop Layers',phone:'+234 807 555 6666',location:'Osun',notes:''}],
  orders:[{id:'FO001',customerId:'FC001',date:'2025-01-21',product:'Broiler Starter',qty:2000,unitPrice:846,status:'Fulfilled',deliveryDate:'2025-01-21',notes:''},{id:'FO002',customerId:'FC002',date:'2025-01-25',product:'Layer Mash',qty:3000,unitPrice:780,status:'Pending',deliveryDate:'2025-01-30',notes:''}],
  suppliers:[{id:'FS001',name:'Oyo Grains Ltd',material:'Maize',phone:'+234 802 777 8888',location:'Saki, Oyo',lastPrice:480,unit:'kg',notes:'Reliable, bulk discounts'},{id:'FS002',name:'SoyaSource Nigeria',material:'Soybean Meal',phone:'+234 809 999 0000',location:'Ibadan',lastPrice:720,unit:'kg',notes:''}],
...
function FMCmdCenter({state,dataMode}){
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      {/* Visual KPI reports */}
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
        <ChartCard title="Production Output — recent batches" subtitle="kg produced">
          <MiniBarChart data={productionBatches.slice(0,7).reverse().map(b=>({label:String(b.batchNo).slice(-4),value:b.actualQty||0}))} color={T.chartBlue} unit="kg"/>
        </ChartCard>
        <ChartCard title="Batch Efficiency — recent" subtitle="% of target">
          <MiniLineChart data={completed.slice(0,8).reverse().map((b,i)=>({label:String(b.batchNo).slice(-4),value:b.targetQty>0?Math.round(b.actualQty/b.targetQty*100):0}))} color={T.chartBlue} unit="%"/>
        </ChartCard>
      </div>
...
function FeedMillModule({navSignal,capacity,license,activeUser,onUpdateLicense,dataMode,onSwitchToLive,onRestoreTraining,initialSeed}){
  ...
  // ── Cross-device sync for FeedMillOS (same proven pattern) ──
  ...
  useEffect(()=>{
    let cancelled=false;
    (async()=>{
      if(!__fmFarmId){__fmReady.current=true;return;}
      try{
        const mod=await import('../sync/collectionSync.js');
        for(const coll of FM_SYNC_COLLECTIONS){
          const {records}=await mod.loadCollection(__fmFarmId, 'fm_'+coll);
          if(cancelled)return;
          if(records&&records.length){
            dispatch({type:'__SYNC_SET_COLLECTION', collection:coll, p:records});
            __fmSig.current[coll]={};
            records.forEach(r=>{if(r&&r.id)__fmSig.current[coll][r.id]=JSON.stringify(r);});
          }
        }
      }catch(_){}
      __fmReady.current=true;
    })();
    return ()=>{cancelled=true;};
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[__fmFarmId]);
  useEffect(()=>{
    if(!__fmFarmId||!__fmReady.current)return;
    let cancelled=false;
    (async()=>{
      try{
        const mod=await import('../sync/collectionSync.js');
        for(const coll of FM_SYNC_COLLECTIONS){
          if(!__fmSig.current[coll])__fmSig.current[coll]={};
          await mod.syncCollectionDiff(__fmFarmId, 'fm_'+coll, fm[coll]||[], __fmSig.current[coll]);
          if(cancelled)return;
        }
      }catch(_){}
    })();
...
function Dashboard({license,activeUser,onLogout,addAudit,auditLog,dataMode,onSwitchToLive,onRestoreTraining,switchKey,onUpdateLicense,demoRec}){
  ...
  // navSignal increments on each notification-driven navigation so modules
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',height:'100vh',background:T.bg1,overflow:'hidden',fontFamily:'inherit'}}>
      <div style={{background:T.bg0,borderBottom:`1px solid ${T.line}`,flexShrink:0,paddingTop:'calc(env(safe-area-inset-top, 0px) + 10px)'}}>
        <div style={{height:56,display:'flex',alignItems:'center',justifyContent:'space-between',paddingLeft:'var(--ps-gutter-x)',paddingRight:'var(--ps-gutter-x)',maxWidth:'var(--ps-max-width)',marginLeft:'auto',marginRight:'auto',width:'100%',boxSizing:'border-box'}}>
          <div style={{display:'flex',alignItems:'flex-start',gap:10,minWidth:0}}>
            ...
            <div style={{minWidth:0}}>
              ...
              <div style={{fontSize:10,color:T.ink4,marginTop:1,whiteSpace:'nowrap'}}>Powered by <span style={{fontWeight:600,color:T.ink3}}>AgoroX Technologies</span></div>
            </div>
          </div>
        ...
        <div style={{display:'flex',gap:0,overflowX:'auto',borderTop:`1px solid ${T.line}`,WebkitOverflowScrolling:'touch',maxWidth:'var(--ps-max-width)',marginLeft:'auto',marginRight:'auto',width:'100%',paddingLeft:'var(--ps-gutter-x)',paddingRight:'var(--ps-gutter-x)',boxSizing:'border-box'}}>
          ...
          {canViewLicense&&demoActive&&<div style={{display:'flex',alignItems:'center',marginLeft:'auto',paddingRight:4,flexShrink:0}}>
            <div style={{fontSize:11,fontWeight:600,color:T.warn,background:T.warnBg,border:`1px solid ${T.warnLine}`,padding:'3px 9px',whiteSpace:'nowrap'}}>{demoLeft}d demo left</div>
          </div>}
          {canViewLicense&&<button onClick={()=>setActiveId('license')}
            style={{display:'flex',alignItems:'center',gap:6,padding:'9px 14px',border:'none',borderBottom:`2px solid ${activeId==='license'?T.accent:'transparent'}`,background:activeId==='license'?T.accentBg:'transparent',cursor:'pointer',whiteSpace:'nowrap',fontFamily:'inherit',color:activeId==='license'?T.accent:T.ink3,fontWeight:activeId==='license'?600:400,fontSize:12,flexShrink:0,marginLeft:demoActive?0:'auto',transition:'color 0.12s,background 0.12s',marginBottom:-1}}>
      ...
      <main style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',paddingLeft:'var(--ps-gutter-x)',paddingRight:'var(--ps-gutter-x)',paddingTop:'var(--ps-gutter-y)',paddingBottom:'var(--ps-gutter-y)'}} key={activeId} className="au"><div style={{maxWidth:'var(--ps-max-width)',marginLeft:'auto',marginRight:'auto',width:'100%'}}>
        ...
        {activeId==='license'&&canViewLicense&&(<div style={{maxWidth:640,margin:'0 auto',display:'flex',flexDirection:'column',gap:16}}>
          ...
          {!license.paid&&(
            <button onClick={()=>setShowUpgrade(true)} style={{width:'100%',padding:'14px',background:showUpgrade?'#0a3d2e':T.accent,color:'#fff',border:'none',fontSize:14,fontWeight:700,cursor:'pointer',fontFamily:'inherit',letterSpacing:0.2}}>
              {showUpgrade?'✓ Opening upgrade screen…':'Buy License'}
            </button>
          )}
        </div>)}
      </div></main>
...
function PoultrySuiteAfricaCore(){
  ...
  const handleSessionTimeout=useCallback(()=>{
    if(screen==='dashboard'){
      addAudit('Session timeout - locked','System');
      lockNow(); // triggers 6-digit PIN lock via IdleLock, no disruptive modal
    }
  },[screen,addAudit]);
  
  // Session timeout is handled by IdleLock (6-digit PIN), not a warning modal.
  // useSessionTimeout is retained in the codebase but disabled here.
  ...
  return(<>
    ...
    {screen==='login'         &&<PinLoginScreen storedPin={license?.pin} license={license} onSuccess={handleLoginSuccess} onReset={handleResetSetup}/>}
